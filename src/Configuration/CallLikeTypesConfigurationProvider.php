...
final class CallLikeTypesConfigurationProvider
{
            ...
            foreach ($typesByPositionByMethodNames as $methodName => $typesByPosition) {
                foreach ($typesByPosition as $position => $types) {
                    ...
                    if (count($uniqueTypes) === 1) {
                        ...
                        $classMethodTypes[] = new ClassMethodType($className, $methodName, $position, $uniqueTypes[0]);
                        continue;
                    }
                    if (in_array(NullType::class, $uniqueTypes) && count($uniqueTypes) === 2) {
                        ...
                        $classMethodTypes[] = new ClassMethodType($className, $methodName, $position, $typeWithoutNull, \true);
                        continue;
                    }
                    // log invalid type to improve
                    FilesLoader::writeJsonl('debug.json', ['skipped_types' => $uniqueTypes]);
                }
            }
        }
        $this->classMethodTypes = $classMethodTypes;
    ...
    {
        return array_filter($classMethodTypes, function (ClassMethodType $classMethodType) use ($className, $methodName): bool {
