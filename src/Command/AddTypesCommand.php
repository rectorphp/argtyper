...
final class AddTypesCommand extends Command
{
    ...
    private function runPhpStan(array $projectDirs, string $projectPath, bool $isDebug): void
    {
        ...
        $cmd = sprintf('vendor/bin/phpstan analyse %s --configuration=%s --autoload-file=%s', implode(' ', $dirs), realpath(__DIR__ . '/../../config/phpstan-collecting-data.neon'), escapeshellarg($projectPath . '/vendor/autoload.php'));
    ...
    private function runRector(array $projectDirs, bool $isDebug): void
    {
        ...
        $cmd = sprintf('vendor/bin/rector process %s --config=%s --clear-cache', implode(' ', $projectDirs), realpath(__DIR__ . '/../../rector/rector-argtyper.php'));
