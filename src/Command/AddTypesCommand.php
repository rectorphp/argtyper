...
final class AddTypesCommand extends Command
{
    ...
    private function runPhpStan(Project $project, bool $isDebug): void
    {
        ...
        $commands = array_merge(['vendor/bin/phpstan', 'analyse'], $project->getCodeDirectories(), ['--configuration', (string) realpath(__DIR__ . '/../../config/phpstan-collecting-data.neon'), '--autoload-file', (string) realpath(__DIR__ . '/../../bin/autoload.php')]);
    ...
    private function runRector(Project $project, bool $isDebug): void
    {
        ...
        $command = array_merge(['vendor/bin/rector', 'process'], $project->getAbsoluteCodeDirectories(), ['--config', (string) realpath(__DIR__ . '/../../config/rector-argtyper.php'), '--clear-cache']);
