...
use Argtyper202511\Nette\Utils\Strings;
...
final class AddTypesCommand extends Command
{
    ...
    public function __construct(ProjectDirectoryFinder $projectDirectoryFinder, SymfonyStyle $symfonyStyle)
    {
    ...
    private function runPhpStan(array $projectDirs, string $projectPath, bool $isDebug): void
    {
        ...
        $relativeProjectDirs = array_map(function (string $dir) use ($projectPath): string {
            return (string) Strings::after($dir, $projectPath, 1);
        }, $projectDirs);
        sort($relativeProjectDirs);
        // Keep paths the same as in the original script
        $commands = array_merge(['vendor/bin/phpstan', 'analyse'], $relativeProjectDirs, ['--configuration', realpath(__DIR__ . '/../../config/phpstan-collecting-data.neon'), '--autoload-file', realpath(__DIR__ . '/../../bin/autoload.php')]);
    ...
    private function runRector(array $projectDirs, bool $isDebug): void
    {
        ...
        $command = array_merge(['vendor/bin/rector', 'process'], $projectDirs, ['--config', realpath(__DIR__ . '/../../rector/rector-argtyper.php'), '--clear-cache']);
