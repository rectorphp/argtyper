...
final class AddTypesCommand extends Command
{
    /**
     * @readonly
     * @var \Rector\ArgTyper\Helpers\ProjectSourceDirFinder
     */
    private $projectSourceDirFinder;
    /**
     * @readonly
     * @var \Symfony\Component\Console\Style\SymfonyStyle
     */
    private $symfonyStyle;
    public function __construct(ProjectSourceDirFinder $projectSourceDirFinder, SymfonyStyle $symfonyStyle)
    {
        $this->projectSourceDirFinder = $projectSourceDirFinder;
    ...
    private function runPhpStan(array $projectDirs, string $projectPath, bool $isDebug): void
    {
        ...
        $dirs = array_map(\Closure::fromCallable('escapeshellarg'), $projectDirs);
        // Keep paths the same as in the original script
        $cmd = sprintf('vendor/bin/phpstan analyse %s --configuration=%s --autoload-file=%s', implode(' ', $dirs), realpath(__DIR__ . '/../../phpstan-data-collector.neon'), escapeshellarg($projectPath . '/vendor/autoload.php'));
    ...
    private function runRector(array $projectDirs, bool $isDebug): void
    {
        ...
        $cmd = sprintf('vendor/bin/rector process %s --config=%s --clear-cache --autoload-file=%s', implode(' ', $projectDirs), realpath(__DIR__ . '/../../rector-argtyper.php'), escapeshellarg(getcwd() . '/vendor/autoload.php'));
        // show output, so we know what exactly has changed
        $rectorOutput = $this->runShell($cmd, $isDebug, \true);
