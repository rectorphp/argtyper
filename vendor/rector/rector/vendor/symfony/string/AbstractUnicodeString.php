 ...
 */
namespace Argtyper202511\RectorPrefix202511\Symfony\Component\String;

use Argtyper202511\RectorPrefix202511\Symfony\Component\String\Exception\ExceptionInterface;
use Argtyper202511\RectorPrefix202511\Symfony\Component\String\Exception\InvalidArgumentException;
use Argtyper202511\RectorPrefix202511\Symfony\Component\String\Exception\RuntimeException;
/**
...
abstract class AbstractUnicodeString extends AbstractString
{
    ...
    {
        $str = $this->camel();
        $str->string = \mb_strtolower(\preg_replace(['/(\\p{Lu}+)(\\p{Lu}\\p{Ll})/u', '/([\\p{Ll}0-9])(\\p{Lu})/u'], 'Argtyper202511\\1_\\2', $str->string), 'UTF-8');
        return $str;
    }
    /**
     * @return static
    ...
    public function trimPrefix($prefix)
    {
        ...
        if ($prefix instanceof \Traversable) {
            $prefix = \iterator_to_array((\is_array(\is_array($prefix) ? new \ArrayIterator($prefix) : $prefix) ? new \ArrayIterator(\is_array($prefix) ? new \ArrayIterator($prefix) : $prefix) : \is_array($prefix)) ? new \ArrayIterator($prefix) : $prefix, \false);
    ...
    public function trimSuffix($suffix)
    {
        ...
        if ($suffix instanceof \Traversable) {
            $suffix = \iterator_to_array((\is_array(\is_array($suffix) ? new \ArrayIterator($suffix) : $suffix) ? new \ArrayIterator(\is_array($suffix) ? new \ArrayIterator($suffix) : $suffix) : \is_array($suffix)) ? new \ArrayIterator($suffix) : $suffix, \false);
