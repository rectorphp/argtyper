...
use RectorPrefix202511\Symfony\Component\Yaml\Exception\DumpException;
...
class Inline
{
    ...
    {
        self::$exceptionOnInvalidType = (bool) (\RectorPrefix202511\Symfony\Component\Yaml\Yaml::PARSE_EXCEPTION_ON_INVALID_TYPE & $flags);
        self::$objectSupport = (bool) (\RectorPrefix202511\Symfony\Component\Yaml\Yaml::PARSE_OBJECT & $flags);
        self::$objectForMap = (bool) (\RectorPrefix202511\Symfony\Component\Yaml\Yaml::PARSE_OBJECT_FOR_MAP & $flags);
        self::$constantSupport = (bool) (\RectorPrefix202511\Symfony\Component\Yaml\Yaml::PARSE_CONSTANT & $flags);
    ...
    public static function dump($value, int $flags = 0, bool $rootLevel = \false): string
    {
        switch (\true) {
            case \is_resource($value):
                if (\RectorPrefix202511\Symfony\Component\Yaml\Yaml::DUMP_EXCEPTION_ON_INVALID_TYPE & $flags) {
            ...
            case \is_object($value):
                ...
                if (\RectorPrefix202511\Symfony\Component\Yaml\Yaml::DUMP_OBJECT & $flags) {
                    return '!php/object ' . self::dump(serialize($value));
                }
                if (\RectorPrefix202511\Symfony\Component\Yaml\Yaml::DUMP_OBJECT_AS_MAP & $flags && ($value instanceof \stdClass || $value instanceof \ArrayObject)) {
            ...
            case \RectorPrefix202511\Symfony\Component\Yaml\Escaper::requiresDoubleQuoting($value):
            case \RectorPrefix202511\Symfony\Component\Yaml\Yaml::DUMP_FORCE_DOUBLE_QUOTES_ON_VALUES & $flags:
                return \RectorPrefix202511\Symfony\Component\Yaml\Escaper::escapeWithDoubleQuotes($value);
            case \RectorPrefix202511\Symfony\Component\Yaml\Escaper::requiresSingleQuoting($value):
                $singleQuoted = \RectorPrefix202511\Symfony\Component\Yaml\Escaper::escapeWithSingleQuotes($value);
                if (strpos($value, "'") === \false) {
                    return $singleQuoted;
                }
                // Attempt double-quoting the string instead to see if it's more efficient.
                $doubleQuoted = \RectorPrefix202511\Symfony\Component\Yaml\Escaper::escapeWithDoubleQuotes($value);
                return \strlen($doubleQuoted) < \strlen($singleQuoted) ? $doubleQuoted : $singleQuoted;
            case \RectorPrefix202511\Symfony\Component\Yaml\Parser::preg_match('{^[0-9]+[_0-9]*$}', $value):
            case \RectorPrefix202511\Symfony\Component\Yaml\Parser::preg_match(self::getHexRegex(), $value):
            case \RectorPrefix202511\Symfony\Component\Yaml\Parser::preg_match(self::getTimestampRegex(), $value):
                return \RectorPrefix202511\Symfony\Component\Yaml\Escaper::escapeWithSingleQuotes($value);
            default:
                return $value;
        }
    }
    ...
    {
        // array
        if (($value || \RectorPrefix202511\Symfony\Component\Yaml\Yaml::DUMP_EMPTY_ARRAY_AS_SEQUENCE & $flags) && !self::isHash($value)) {
            $output = [];
    ...
    {
        $output = [];
        $keyFlags = $flags & ~\RectorPrefix202511\Symfony\Component\Yaml\Yaml::DUMP_FORCE_DOUBLE_QUOTES_ON_VALUES;
        foreach ($value as $key => $val) {
            if (\is_int($key) && \RectorPrefix202511\Symfony\Component\Yaml\Yaml::DUMP_NUMERIC_KEY_AS_STRING & $flags) {
                $key = (string) $key;
            }
    ...
    {
        if (\RectorPrefix202511\Symfony\Component\Yaml\Yaml::DUMP_NULL_AS_TILDE & $flags) {
            return '~';
        }
        if (\RectorPrefix202511\Symfony\Component\Yaml\Yaml::DUMP_NULL_AS_EMPTY & $flags && !$rootLevel) {
            return '';
        }
        return 'null';
    }
    ...
    public static function parseScalar(string $scalar, int $flags = 0, ?array $delimiters = null, int &$i = 0, bool $evaluate = \true, array &$references = [], ?bool &$isQuoted = null)
    {
        if (\in_array($scalar[$i], ['"', "'"], \true)) {
            ...
            if (!$delimiters) {
                ...
                $i += \strlen($output);
                // remove comments
                if (\RectorPrefix202511\Symfony\Component\Yaml\Parser::preg_match('/[ \t]+#/', $output, $match, \PREG_OFFSET_CAPTURE)) {
                    $output = (string) substr($output, 0, $match[0][1]);
                }
            } elseif (\RectorPrefix202511\Symfony\Component\Yaml\Parser::preg_match('/^(.*?)(' . implode('|', $delimiters) . ')/', (string) substr($scalar, $i), $match)) {
                $output = $match[1];
                $i += \strlen($output);
                $output = trim($output);
            } else {
    ...
    {
        if (!\RectorPrefix202511\Symfony\Component\Yaml\Parser::preg_match('/' . self::REGEX_QUOTED_STRING . '/Au', (string) substr($scalar, $i), $match)) {
            throw new ParseException(\sprintf('Malformed inline YAML string: "%s".', substr($scalar, $i)), self::$parsedLineNumber + 1, $scalar, self::$parsedFilename);
        }
        $output = (string) substr($match[0], 1, -1);
        $unescaper = new \RectorPrefix202511\Symfony\Component\Yaml\Unescaper();
        if ('"' == $scalar[$i]) {
        ...
        while ($i < $len) {
            ...
            switch ($sequence[$i]) {
                ...
                default:
                    ...
                    if (!$isQuoted && \is_string($value) && '' !== $value && '&' === $value[0] && \RectorPrefix202511\Symfony\Component\Yaml\Parser::preg_match(\RectorPrefix202511\Symfony\Component\Yaml\Parser::REFERENCE_PATTERN, $value, $matches)) {
                ...
                switch ($mapping[$i]) {
                    ...
                    default:
                        ...
                        if ('<<' === $key) {
                            ...
                            if (!$isValueQuoted && \is_string($value) && '' !== $value && '&' === $value[0] && !self::isBinaryString($value) && \RectorPrefix202511\Symfony\Component\Yaml\Parser::preg_match(\RectorPrefix202511\Symfony\Component\Yaml\Parser::REFERENCE_PATTERN, $value, $matches)) {
                                $references[$matches['ref']] = $matches['value'];
                                $value = $matches['value'];
                            }
                            if (null !== $tag) {
    ...
    private static function evaluateScalar(string $scalar, int $flags, array &$references = [], ?bool &$isQuotedString = null)
    {
        ...
        switch (\true) {
            ...
            case \in_array($scalar[0], ['+', '-', '.'], \true) || is_numeric($scalar[0]):
                if (\RectorPrefix202511\Symfony\Component\Yaml\Parser::preg_match('{^[+-]?[0-9][0-9_]*$}', $scalar)) {
                ...
                switch (\true) {
                    ...
                    case \RectorPrefix202511\Symfony\Component\Yaml\Parser::preg_match(self::getHexRegex(), $scalar):
                        $scalar = str_replace('_', '', $scalar);
                    ...
                    case '-.inf' === $scalarLower:
                        return log(0);
                    case \RectorPrefix202511\Symfony\Component\Yaml\Parser::preg_match('/^(-|\+)?[0-9][0-9_]*(\.[0-9_]+)?$/', $scalar):
                        return (float) str_replace('_', '', $scalar);
                    case \RectorPrefix202511\Symfony\Component\Yaml\Parser::preg_match(self::getTimestampRegex(), $scalar):
                        try {
                        ...
                        if (\RectorPrefix202511\Symfony\Component\Yaml\Yaml::PARSE_DATETIME & $flags) {
                            return $time;
                        }
    ...
    private static function parseTag(string $value, int &$i, int $flags): ?string
    {
        ...
        if ('' === $tag || \RectorPrefix202511\Symfony\Component\Yaml\Yaml::PARSE_CUSTOM_TAGS & $flags) {
            return $tag;
        }
    ...
    public static function evaluateBinaryScalar(string $scalar): string
    {
        ...
        if (!\RectorPrefix202511\Symfony\Component\Yaml\Parser::preg_match('#^[A-Z0-9+/]+={0,2}$#i', $parsedBinaryData)) {
