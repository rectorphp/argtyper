...
class Parser
{
    ...
    private function doParse(string $value, int $flags)
    {
        ...
        do {
            ...
            \RectorPrefix202511\Symfony\Component\Yaml\Inline::initialize($flags, $this->getRealCurrentLineNb(), $this->filename);
            ...
            if ('-' === $this->currentLine[0] && self::preg_match('#^\-((?P<leadspaces>\s+)(?P<value>.+))?$#u', rtrim($this->currentLine), $values)) {
                ...
                } else if (isset($values['leadspaces']) && ('!' === $values['value'][0] || self::preg_match('#^(?P<key>' . \RectorPrefix202511\Symfony\Component\Yaml\Inline::REGEX_QUOTED_STRING . '|[^ \'"\{\[].*?) *\:(\s+(?P<value>.+?))?\s*$#u', $this->trimTag($values['value']), $matches))) {
                    $block = $values['value'];
            ...
            } elseif (self::preg_match('#^(?P<key>(?:![^\s]++\s++)?(?:' . \RectorPrefix202511\Symfony\Component\Yaml\Inline::REGEX_QUOTED_STRING . '|[^ \'"\[\{!].*?)) *\:(( |\t)++(?P<value>.+))?$#u', rtrim($this->currentLine), $values) && (strpos($values['key'], ' #') === \false || \in_array($values['key'][0], ['"', "'"]))) {
                if ($context && 'sequence' == $context) {
                    throw new ParseException('You cannot define a mapping item when in a sequence.', $this->currentLineNb + 1, $this->currentLine, $this->filename);
                }
                $context = 'mapping';
                try {
                    $key = \RectorPrefix202511\Symfony\Component\Yaml\Inline::parseScalar($values['key']);
                ...
                if ('<<' === $key && (!isset($values['value']) || '&' !== $values['value'][0] || !self::preg_match('#^&(?P<ref>[^ ]+)#u', $values['value'], $refMatches))) {
                    ...
                    if (isset($values['value'][0]) && '*' === $values['value'][0]) {
                        ...
                        if (\RectorPrefix202511\Symfony\Component\Yaml\Yaml::PARSE_OBJECT_FOR_MAP & $flags && $refValue instanceof \stdClass) {
                        ...
                        if (\RectorPrefix202511\Symfony\Component\Yaml\Yaml::PARSE_OBJECT_FOR_MAP & $flags && $parsed instanceof \stdClass) {
                            $parsed = (array) $parsed;
                        }
                        if (!\is_array($parsed)) {
                            throw new ParseException('YAML merge keys used with a scalar value instead of an array.', $this->getRealCurrentLineNb() + 1, $this->currentLine, $this->filename);
                        ...
                        if (isset($parsed[0])) {
                            ...
                            foreach ($parsed as $parsedItem) {
                                if (\RectorPrefix202511\Symfony\Component\Yaml\Yaml::PARSE_OBJECT_FOR_MAP & $flags && $parsedItem instanceof \stdClass) {
                ...
                if ($mergeNode) {
                    ...
                    if (!$this->isNextLineIndented() && !$this->isNextLineUnIndentedCollection()) {
                        ...
                        if ('<<' === $key) {
                            ...
                            if (\RectorPrefix202511\Symfony\Component\Yaml\Yaml::PARSE_OBJECT_FOR_MAP & $flags && $value instanceof \stdClass) {
                                $value = (array) $value;
                            }
                            $data += $value;
                ...
                try {
                    return \RectorPrefix202511\Symfony\Component\Yaml\Inline::parse($this->lexInlineQuotedString(), $flags, $this->refs);
                } catch (ParseException $e) {
                    $e->setParsedLine($this->getRealCurrentLineNb() + 1);
                    $e->setSnippet($this->currentLine);
                    throw $e;
                ...
                try {
                    $parsedMapping = \RectorPrefix202511\Symfony\Component\Yaml\Inline::parse($this->lexInlineMapping(), $flags, $this->refs);
                ...
                try {
                    $parsedSequence = \RectorPrefix202511\Symfony\Component\Yaml\Inline::parse($this->lexInlineSequence(), $flags, $this->refs);
                    while ($this->moveToNextLine()) {
                        if (!$this->isCurrentLineEmpty()) {
                            throw new ParseException('Unable to parse.', $this->getRealCurrentLineNb() + 1, $this->currentLine, $this->filename);
                        }
                ...
                if (\is_string($value) && $this->lines[0] === trim($value)) {
                    try {
                        $value = \RectorPrefix202511\Symfony\Component\Yaml\Inline::parse($this->lines[0], $flags, $this->refs);
                    } catch (ParseException $e) {
                        $e->setParsedLine($this->getRealCurrentLineNb() + 1);
                        $e->setSnippet($this->currentLine);
                        throw $e;
                ...
                if (0 === $this->currentLineNb) {
                    ...
                    try {
                        return \RectorPrefix202511\Symfony\Component\Yaml\Inline::parse(trim($value));
        ...
        if (\RectorPrefix202511\Symfony\Component\Yaml\Yaml::PARSE_OBJECT_FOR_MAP & $flags && 'mapping' === $context && !\is_object($data)) {
            $object = new \stdClass();
    ...
    private function parseValue(string $value, int $flags, string $context)
    {
        ...
        if (\in_array($value[0], ['!', '|', '>'], \true) && self::preg_match('/^(?:' . self::TAG_PATTERN . ' +)?' . self::BLOCK_SCALAR_HEADER_PATTERN . '$/', $value, $matches)) {
            ...
            if ('' !== $matches['tag'] && '!' !== $matches['tag']) {
                if ('!!binary' === $matches['tag']) {
                    return \RectorPrefix202511\Symfony\Component\Yaml\Inline::evaluateBinaryScalar($data);
                }
        ...
        try {
            if ('' !== $value && '{' === $value[0]) {
                ...
                return \RectorPrefix202511\Symfony\Component\Yaml\Inline::parse($this->lexInlineMapping($cursor), $flags, $this->refs);
            } elseif ('' !== $value && '[' === $value[0]) {
                $cursor = \strlen(rtrim($this->currentLine)) - \strlen(rtrim($value));
                return \RectorPrefix202511\Symfony\Component\Yaml\Inline::parse($this->lexInlineSequence($cursor), $flags, $this->refs);
            }
            switch ($value[0] ?? '') {
                case '"':
                case "'":
                    $cursor = \strlen(rtrim($this->currentLine)) - \strlen(rtrim($value));
                    $parsedValue = \RectorPrefix202511\Symfony\Component\Yaml\Inline::parse($this->lexInlineQuotedString($cursor), $flags, $this->refs);
                ...
                default:
                    ...
                    \RectorPrefix202511\Symfony\Component\Yaml\Inline::$parsedLineNumber = $this->getRealCurrentLineNb();
                    $parsedValue = \RectorPrefix202511\Symfony\Component\Yaml\Inline::parse($value, $flags, $this->refs);
    ...
    private function getLineTag(string $value, int $flags, bool $nextLineCheck = \true): ?string
    {
        ...
        if (\RectorPrefix202511\Symfony\Component\Yaml\Yaml::PARSE_CUSTOM_TAGS & $flags) {
            return $tag;
        }
