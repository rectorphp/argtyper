...
final class RunCommandMessageHandler
{
    /**
     * @readonly
     * @var \RectorPrefix202512\Symfony\Component\Console\Application
     */
    private $application;
    ...
    public function __invoke(\RectorPrefix202512\Symfony\Component\Console\Messenger\RunCommandMessage $message): \RectorPrefix202512\Symfony\Component\Console\Messenger\RunCommandContext
    {
        ...
        try {
            ...
            throw new RunCommandFailedException($e, new \RectorPrefix202512\Symfony\Component\Console\Messenger\RunCommandContext($message, Command::FAILURE, $output->fetch()));
        }
        if ($message->throwOnFailure && Command::SUCCESS !== $exitCode) {
            throw new RunCommandFailedException(\sprintf('Command "%s" exited with code "%s".', $message->input, $exitCode), new \RectorPrefix202512\Symfony\Component\Console\Messenger\RunCommandContext($message, $exitCode, $output->fetch()));
        }
        return new \RectorPrefix202512\Symfony\Component\Console\Messenger\RunCommandContext($message, $exitCode, $output->fetch());
    }
}
