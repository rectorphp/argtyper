...
final class ProgressBar
{
    ...
    /**
     * @var \RectorPrefix202511\Symfony\Component\Console\Cursor
     */
    private $cursor;
    /**
     * @var mixed[]
    ...
    public function setMaxSteps(int $max): void
    {
        $this->format = null;
        $this->max = max(0, $max);
        $this->stepWidth = $this->max ? \RectorPrefix202511\Symfony\Component\Console\Helper\Helper::width((string) $this->max) : 4;
    }
    /**
        ...
        if ($this->overwrite) {
            if (null !== $this->previousMessage) {
                if ($this->output instanceof ConsoleSectionOutput) {
                    ...
                    $lastLineWithoutDecoration = \RectorPrefix202511\Symfony\Component\Console\Helper\Helper::removeDecoration($this->output->getFormatter(), end($messageLines) ?? '');
                    ...
                    foreach ($messageLines as $messageLine) {
                        $messageLineLength = \RectorPrefix202511\Symfony\Component\Console\Helper\Helper::width(\RectorPrefix202511\Symfony\Component\Console\Helper\Helper::removeDecoration($this->output->getFormatter(), $messageLine));
    ...
    private static function initPlaceholderFormatters(): array
    {
        return ['bar' => function (self $bar, OutputInterface $output) {
            ...
            if ($completeBars < $bar->getBarWidth()) {
                $emptyBars = $bar->getBarWidth() - $completeBars - \RectorPrefix202511\Symfony\Component\Console\Helper\Helper::length(\RectorPrefix202511\Symfony\Component\Console\Helper\Helper::removeDecoration($output->getFormatter(), $bar->getProgressCharacter()));
                $display .= $bar->getProgressCharacter() . str_repeat($bar->getEmptyBarCharacter(), $emptyBars);
            }
            return $display;
        }, 'elapsed' => function (self $bar) {
            return \RectorPrefix202511\Symfony\Component\Console\Helper\Helper::formatTime(time() - $bar->getStartTime(), 2);
        }, 'remaining' => function (self $bar) {
            if (!$bar->getMaxSteps()) {
                throw new LogicException('Unable to display the remaining time if the maximum number of steps is not set.');
            }
            return \RectorPrefix202511\Symfony\Component\Console\Helper\Helper::formatTime($bar->getRemaining(), 2);
        }, 'estimated' => function (self $bar) {
            if (!$bar->getMaxSteps()) {
                throw new LogicException('Unable to display the estimated time if the maximum number of steps is not set.');
            }
            return \RectorPrefix202511\Symfony\Component\Console\Helper\Helper::formatTime($bar->getEstimated(), 2);
        }, 'memory' => function (self $bar) {
            return \RectorPrefix202511\Symfony\Component\Console\Helper\Helper::formatMemory(memory_get_usage(\true));
    ...
    private function buildLine(): string
    {
        ...
        $linesLength = array_map(function ($subLine) {
            return \RectorPrefix202511\Symfony\Component\Console\Helper\Helper::width(\RectorPrefix202511\Symfony\Component\Console\Helper\Helper::removeDecoration($this->output->getFormatter(), rtrim($subLine, "\r")));
        }, explode("\n", $line));
