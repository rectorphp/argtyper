...
trait SmartObject
{
    ...
    public function __call(string $name, array $args)
    {
        ...
        if (ObjectHelpers::hasProperty($class, $name) === 'event') {
            ...
            if (is_iterable($handlers)) {
                ...
                throw new \RectorPrefix202511\Nette\UnexpectedValueException("Property {$class}::\${$name} must be iterable or null, " . get_debug_type($handlers) . ' given.');
            }
            return null;
        }
    ...
    public function &__get(string $name)
    {
        ...
        if ($prop = ObjectHelpers::getMagicProperties($class)[$name] ?? null) {
            // property getter
            if (!($prop & 0b1)) {
                throw new \RectorPrefix202511\Nette\MemberAccessException("Cannot read a write-only property {$class}::\${$name}.");
            }
    ...
    public function __set(string $name, $value): void
    {
        ...
        if (ObjectHelpers::hasProperty($class, $name)) {
            ...
            // property setter
            if (!($prop & 0b1000)) {
                throw new \RectorPrefix202511\Nette\MemberAccessException("Cannot write to a read-only property {$class}::\${$name}.");
            }
            $m = 'set' . ucfirst($name);
            if ($prop & 0b10000) {
                $trace = debug_backtrace(0, 1)[0];
    ...
    public function __unset(string $name): void
    {
        $class = static::class;
        if (!ObjectHelpers::hasProperty($class, $name)) {
            throw new \RectorPrefix202511\Nette\MemberAccessException("Cannot unset the property {$class}::\${$name}.");
        }
    }
