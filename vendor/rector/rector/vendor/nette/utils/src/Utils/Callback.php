...
final class Callback
{
    ...
    public static function invokeSafe(string $function, array $args, callable $onError)
    {
        $prev = set_error_handler(function ($severity, $message, $file) use ($onError, &$prev, $function): ?bool {
            if ($file === __FILE__) {
                $msg = ini_get('html_errors') ? \RectorPrefix202511\Nette\Utils\Html::htmlToText($message) : $message;
    ...
    public static function toReflection($callable)
    {
        ...
        if (is_string($callable) && strpos($callable, '::') !== \false) {
            return new \RectorPrefix202511\Nette\Utils\ReflectionMethod(...explode('::', $callable, 2));
        } elseif (is_array($callable)) {
            return new \RectorPrefix202511\Nette\Utils\ReflectionMethod($callable[0], $callable[1]);
        } elseif (is_object($callable) && !$callable instanceof \Closure) {
            return new \RectorPrefix202511\Nette\Utils\ReflectionMethod($callable, '__invoke');
        } else {
