...
class Finder implements \IteratorAggregate
{
    ...
    private function addMask(array $masks, string $mode)
    {
        foreach ($masks as $mask) {
            $mask = \RectorPrefix202511\Nette\Utils\FileSystem::unixSlashes($mask);
            if ($mode === 'dir') {
                $mask = rtrim($mask, '/');
            }
    ...
    private function addLocation(array $paths, string $ext): void
    {
        foreach ($paths as $path) {
            ...
            $path = rtrim(\RectorPrefix202511\Nette\Utils\FileSystem::unixSlashes($path), '/');
            $this->in[] = $path . $ext;
        }
    }
    /**
    ...
    {
        $this->sort = function (\RectorPrefix202511\Nette\Utils\FileInfo $a, \RectorPrefix202511\Nette\Utils\FileInfo $b): int {
    ...
    public function exclude($masks)
    {
        ...
        foreach ($masks as $mask) {
            ...
            $filter = function (\RectorPrefix202511\Nette\Utils\FileInfo $file) use ($end, $re): bool {
                return $end && !$file->isDir() || !preg_match($re, \RectorPrefix202511\Nette\Utils\FileSystem::unixSlashes($file->getRelativePathname()));
            };
    ...
    public function size(string $operator, ?int $size = null)
    {
        ...
        return $this->filter(function (\RectorPrefix202511\Nette\Utils\FileInfo $file) use ($operator, $size): bool {
            return !$file->isFile() || \RectorPrefix202511\Nette\Utils\Helpers::compare($file->getSize(), $operator, $size);
        });
    }
    /**
    ...
    public function date(string $operator, $date = null)
    {
        ...
        $date = \RectorPrefix202511\Nette\Utils\DateTime::from($date)->getTimestamp();
        return $this->filter(function (\RectorPrefix202511\Nette\Utils\FileInfo $file) use ($operator, $date): bool {
            return !$file->isFile() || \RectorPrefix202511\Nette\Utils\Helpers::compare($file->getMTime(), $operator, $date);
        });
    }
    ...
    public function getIterator(): \Generator
    {
        ...
        foreach ($this->appends as $item) {
            if ($item instanceof self) {
                ...
                $item = \RectorPrefix202511\Nette\Utils\FileSystem::platformSlashes($item);
                yield $item => new \RectorPrefix202511\Nette\Utils\FileInfo($item);
            }
        }
    }
    /**
    ...
    private function traverseDir(string $dir, array $searches, array $subdirs = []): \Generator
    {
        ...
        $files = $this->convertToFiles($pathNames, implode('/', $subdirs), \RectorPrefix202511\Nette\Utils\FileSystem::isAbsolute($dir));
        if ($this->sort) {
        ...
        foreach ($files as $file) {
            ...
            $relativePathname = \RectorPrefix202511\Nette\Utils\FileSystem::unixSlashes($file->getRelativePathname());
            foreach ($searches as $search) {
    ...
    private function convertToFiles(iterable $pathNames, string $relativePath, bool $absolute): \Generator
    {
        foreach ($pathNames as $pathName) {
            ...
            $pathName = \RectorPrefix202511\Nette\Utils\FileSystem::platformSlashes($pathName);
            yield new \RectorPrefix202511\Nette\Utils\FileInfo($pathName, $relativePath);
        }
    }
    private function proveFilters(array $filters, \RectorPrefix202511\Nette\Utils\FileInfo $file, array &$cache): bool
    {
    ...
    private function buildPlan(): array
    {
        ...
        foreach ($this->find as [$mask, $mode]) {
            $splits = [];
            if (\RectorPrefix202511\Nette\Utils\FileSystem::isAbsolute($mask)) {
                if ($this->in) {
    ...
    private function buildPattern(string $mask): string
    {
        ...
        return '#' . $anchor . $pattern . '$#D' . (\RectorPrefix202511\Nette\Utils\Helpers::IsWindows ? 'i' : '');
    }
}
