...
class Image
{
    ...
    /** image types */
    public const JPEG = \RectorPrefix202511\Nette\Utils\ImageType::JPEG, PNG = \RectorPrefix202511\Nette\Utils\ImageType::PNG, GIF = \RectorPrefix202511\Nette\Utils\ImageType::GIF, WEBP = \RectorPrefix202511\Nette\Utils\ImageType::WEBP, AVIF = \RectorPrefix202511\Nette\Utils\ImageType::AVIF, BMP = \RectorPrefix202511\Nette\Utils\ImageType::BMP;
    ...
    private const Formats = [\RectorPrefix202511\Nette\Utils\ImageType::JPEG => 'jpeg', \RectorPrefix202511\Nette\Utils\ImageType::PNG => 'png', \RectorPrefix202511\Nette\Utils\ImageType::GIF => 'gif', \RectorPrefix202511\Nette\Utils\ImageType::WEBP => 'webp', \RectorPrefix202511\Nette\Utils\ImageType::AVIF => 'avif', \RectorPrefix202511\Nette\Utils\ImageType::BMP => 'bmp'];
    /**
     * @var \GdImage
     */
    private $image;
    ...
    public static function fromFile(string $file, ?int &$type = null)
    {
        ...
        if (!$type) {
            throw new \RectorPrefix202511\Nette\Utils\UnknownImageFileException(is_file($file) ? "Unknown type of file '{$file}'." : "File '{$file}' not found.");
        }
    ...
    public static function fromString(string $s, ?int &$type = null)
    {
        ...
        if (!$type) {
            throw new \RectorPrefix202511\Nette\Utils\UnknownImageFileException('Unknown type of image.');
        }
    ...
    {
        $errors = [];
        $res = \RectorPrefix202511\Nette\Utils\Callback::invokeSafe($func, [$arg], function (string $message) use (&$errors): void {
            $errors[] = $message;
        });
        if (!$res) {
            throw new \RectorPrefix202511\Nette\Utils\ImageException($message . ' Errors: ' . implode(', ', $errors));
        } elseif ($errors) {
    ...
    {
        $extensions = array_flip(self::Formats) + ['jpg' => \RectorPrefix202511\Nette\Utils\ImageType::JPEG];
    ...
    public static function isTypeSupported(int $type): bool
    {
        self::ensureExtension();
        switch ($type) {
            case \RectorPrefix202511\Nette\Utils\ImageType::JPEG:
                return IMG_JPG;
            case \RectorPrefix202511\Nette\Utils\ImageType::PNG:
                return IMG_PNG;
            case \RectorPrefix202511\Nette\Utils\ImageType::GIF:
                return IMG_GIF;
            case \RectorPrefix202511\Nette\Utils\ImageType::WEBP:
                return IMG_WEBP;
            case \RectorPrefix202511\Nette\Utils\ImageType::AVIF:
                return 256;
            case \RectorPrefix202511\Nette\Utils\ImageType::BMP:
                return IMG_BMP;
            default:
                return 0;
        }
    ...
    public static function getSupportedTypes(): array
    {
        ...
        $flag = imagetypes();
        return array_filter([
            $flag & IMG_GIF ? \RectorPrefix202511\Nette\Utils\ImageType::GIF : null,
            $flag & IMG_JPG ? \RectorPrefix202511\Nette\Utils\ImageType::JPEG : null,
            $flag & IMG_PNG ? \RectorPrefix202511\Nette\Utils\ImageType::PNG : null,
            $flag & IMG_WEBP ? \RectorPrefix202511\Nette\Utils\ImageType::WEBP : null,
            $flag & 256 ? \RectorPrefix202511\Nette\Utils\ImageType::AVIF : null,
            // IMG_AVIF
            $flag & IMG_BMP ? \RectorPrefix202511\Nette\Utils\ImageType::BMP : null,
        ]);
    }
    /**
     * Wraps GD image.
    ...
    public function resize($width, $height, int $mode = self::OrSmaller)
    {
        ...
        if ($newWidth !== $this->getWidth() || $newHeight !== $this->getHeight()) {
            // resize
            $newImage = static::fromBlank($newWidth, $newHeight, \RectorPrefix202511\Nette\Utils\ImageColor::rgb(0, 0, 0, 0))->getImageResource();
    ...
    public function crop($left, $top, $width, $height)
    {
        ...
        if (gd_info()['GD Version'] === 'bundled (2.1.0 compatible)') {
            ...
            $newImage = static::fromBlank($r['width'], $r['height'], \RectorPrefix202511\Nette\Utils\ImageColor::rgb(0, 0, 0, 0))->getImageResource();
     ...
     * Draw a rectangle.
     */
    public function rectangleWH(int $x, int $y, int $width, int $height, \RectorPrefix202511\Nette\Utils\ImageColor $color): void
    {
        if ($width !== 0 && $height !== 0) {
     ...
     */
    public function filledRectangleWH(int $x, int $y, int $width, int $height, \RectorPrefix202511\Nette\Utils\ImageColor $color): void
    {
        if ($width !== 0 && $height !== 0) {
     ...
     * @param  ImageType::*  $type
     */
    public function toString(int $type = \RectorPrefix202511\Nette\Utils\ImageType::JPEG, ?int $quality = null): string
    {
        return \RectorPrefix202511\Nette\Utils\Helpers::capture(function () use ($type, $quality): void {
     ...
     * @throws ImageException
     */
    public function send(int $type = \RectorPrefix202511\Nette\Utils\ImageType::JPEG, ?int $quality = null): void
    {
    ...
    private function output(int $type, ?int $quality, ?string $file = null): void
    {
        ...
        \RectorPrefix202511\Nette\Utils\Callback::invokeSafe('image' . self::Formats[$type], $args, function (string $message) use ($file): void {
            if ($file !== null) {
                @unlink($file);
            }
            throw new \RectorPrefix202511\Nette\Utils\ImageException($message);
        });
    }
    /**
    ...
    public function __call(string $name, array $args)
    {
        ...
        if (!function_exists($function)) {
            \RectorPrefix202511\Nette\Utils\ObjectHelpers::strictCall(static::class, $name);
        }
        foreach ($args as $key => $value) {
            if ($value instanceof self) {
                $args[$key] = $value->getImageResource();
            } elseif ($value instanceof \RectorPrefix202511\Nette\Utils\ImageColor || is_array($value) && isset($value['red'])) {
    ...
    {
        $color = $color instanceof \RectorPrefix202511\Nette\Utils\ImageColor ? $color->toRGBA() : array_values($color);
