...
final class FileSystem
{
    ...
    public static function createDir(string $dir, int $mode = 0777): void
    {
        if (!is_dir($dir) && !@mkdir($dir, $mode, \true) && !is_dir($dir)) {
            ...
            throw new Nette\IOException(sprintf("Unable to create directory '%s' with mode %s. %s", self::normalizePath($dir), decoct($mode), \RectorPrefix202511\Nette\Utils\Helpers::getLastError()));
        }
    }
    /**
    ...
    public static function copy(string $origin, string $target, bool $overwrite = \true): void
    {
        if (stream_is_local($origin) && !file_exists($origin)) {
            ...
            if (@stream_copy_to_stream(static::open($origin, 'rb'), static::open($target, 'wb')) === \false) {
                ...
                throw new Nette\IOException(sprintf("Unable to copy file '%s' to '%s'. %s", self::normalizePath($origin), self::normalizePath($target), \RectorPrefix202511\Nette\Utils\Helpers::getLastError()));
            }
        }
    }
    /**
    ...
    public static function open(string $path, string $mode)
    {
        ...
        // @ is escalated to exception
        if (!$f) {
            throw new Nette\IOException(sprintf("Unable to open file '%s'. %s", self::normalizePath($path), \RectorPrefix202511\Nette\Utils\Helpers::getLastError()));
        }
        return $f;
    }
    /**
    ...
    public static function delete(string $path): void
    {
        if (is_file($path) || is_link($path)) {
            ...
            if (!@$func($path)) {
                // @ is escalated to exception
                throw new Nette\IOException(sprintf("Unable to delete '%s'. %s", self::normalizePath($path), \RectorPrefix202511\Nette\Utils\Helpers::getLastError()));
            }
        } elseif (is_dir($path)) {
            ...
            if (!@rmdir($path)) {
                // @ is escalated to exception
                throw new Nette\IOException(sprintf("Unable to delete directory '%s'. %s", self::normalizePath($path), \RectorPrefix202511\Nette\Utils\Helpers::getLastError()));
            }
        }
    }
    /**
    ...
    public static function rename(string $origin, string $target, bool $overwrite = \true): void
    {
        if (!$overwrite && file_exists($target)) {
            ...
            if (!@rename($origin, $target)) {
                // @ is escalated to exception
                throw new Nette\IOException(sprintf("Unable to rename file or directory '%s' to '%s'. %s", self::normalizePath($origin), self::normalizePath($target), \RectorPrefix202511\Nette\Utils\Helpers::getLastError()));
            }
        }
    }
    /**
    ...
    public static function read(string $file): string
    {
        ...
        // @ is escalated to exception
        if ($content === \false) {
            throw new Nette\IOException(sprintf("Unable to read file '%s'. %s", self::normalizePath($file), \RectorPrefix202511\Nette\Utils\Helpers::getLastError()));
        }
        return $content;
    }
    /**
    ...
    public static function readLines(string $file, bool $stripNewLines = \true): \Generator
    {
        return (function ($f) use ($file, $stripNewLines) {
            $counter = 0;
            do {
                $line = \RectorPrefix202511\Nette\Utils\Callback::invokeSafe('fgets', [$f], function ($error) use ($file) {
    ...
    public static function write(string $file, string $content, ?int $mode = 0666): void
    {
        ...
        if (@file_put_contents($file, $content) === \false) {
            // @ is escalated to exception
            throw new Nette\IOException(sprintf("Unable to write file '%s'. %s", self::normalizePath($file), \RectorPrefix202511\Nette\Utils\Helpers::getLastError()));
        }
        if ($mode !== null && !@chmod($file, $mode)) {
            // @ is escalated to exception
            throw new Nette\IOException(sprintf("Unable to chmod file '%s' to mode %s. %s", self::normalizePath($file), decoct($mode), \RectorPrefix202511\Nette\Utils\Helpers::getLastError()));
        }
    }
    /**
    ...
    public static function makeWritable(string $path, int $dirMode = 0777, int $fileMode = 0666): void
    {
        if (is_file($path)) {
            if (!@chmod($path, $fileMode)) {
                // @ is escalated to exception
                throw new Nette\IOException(sprintf("Unable to chmod file '%s' to mode %s. %s", self::normalizePath($path), decoct($fileMode), \RectorPrefix202511\Nette\Utils\Helpers::getLastError()));
            }
        } elseif (is_dir($path)) {
            foreach (new \FilesystemIterator($path) as $item) {
            ...
            if (!@chmod($path, $dirMode)) {
                // @ is escalated to exception
                throw new Nette\IOException(sprintf("Unable to chmod directory '%s' to mode %s. %s", self::normalizePath($path), decoct($dirMode), \RectorPrefix202511\Nette\Utils\Helpers::getLastError()));
            }
        } else {
