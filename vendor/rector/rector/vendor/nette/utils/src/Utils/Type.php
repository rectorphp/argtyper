...
final class Type
{
    ...
    private static function fromReflectionType(\ReflectionType $type, $of, bool $asObject)
    {
        if ($type instanceof \ReflectionNamedType) {
            ...
            throw new Nette\InvalidStateException('Unexpected type of ' . \RectorPrefix202512\Nette\Utils\Reflection::toString($of));
        }
    }
    /**
    ...
    {
        if (!\RectorPrefix202512\Nette\Utils\Validators::isTypeDeclaration($type)) {
    ...
    public static function resolve(string $type, $of): string
    {
        ...
        if ($of instanceof \ReflectionFunction) {
            ...
            return ($of instanceof \RectorPrefix202512\Nette\Utils\ReflectionMethod ? $of->getOriginalClass() : $of->getDeclaringClass())->name;
    ...
    {
        return $this->simple && \RectorPrefix202512\Nette\Utils\Validators::isBuiltinType($this->types[0]);
    }
    /**
    ...
    {
        return $this->simple && \RectorPrefix202512\Nette\Utils\Validators::isClassKeyword($this->types[0]);
    }
    /**
    ...
    public function allows($type): bool
    {
        ...
        $type = is_string($type) ? self::fromString($type) : $type;
        return $type->isUnion() ? \RectorPrefix202512\Nette\Utils\Arrays::every($type->types, function ($t) {
    ...
    {
        return $this->isUnion() ? \RectorPrefix202512\Nette\Utils\Arrays::some($this->types, function ($t) use ($givenTypes) {
    ...
    {
        return \RectorPrefix202512\Nette\Utils\Arrays::every($ourTypes, function ($ourType) use ($givenTypes) {
            return \RectorPrefix202512\Nette\Utils\Arrays::some($givenTypes, function ($givenType) use ($ourType) {
                return \RectorPrefix202512\Nette\Utils\Validators::isBuiltinType($ourType) ? strcasecmp($ourType, $givenType) === 0 : is_a($givenType, $ourType, \true);
            });
        });
    }
}
