...
final class Type
{
    ...
    private static function fromReflectionType(\ReflectionType $type, $of, bool $asObject)
    {
        if ($type instanceof \ReflectionNamedType) {
            ...
            throw new Nette\InvalidStateException('Unexpected type of ' . \RectorPrefix202511\Nette\Utils\Reflection::toString($of));
        }
    }
    /**
    ...
    {
        if (!\RectorPrefix202511\Nette\Utils\Validators::isTypeDeclaration($type)) {
    ...
    public static function resolve(string $type, $of): string
    {
        ...
        if ($of instanceof \ReflectionFunction) {
            ...
            return ($of instanceof \RectorPrefix202511\Nette\Utils\ReflectionMethod ? $of->getOriginalClass() : $of->getDeclaringClass())->name;
    ...
    {
        return $this->simple && \RectorPrefix202511\Nette\Utils\Validators::isBuiltinType($this->types[0]);
    }
    /**
    ...
    {
        return $this->simple && \RectorPrefix202511\Nette\Utils\Validators::isClassKeyword($this->types[0]);
    }
    /**
    ...
    public function allows(string $subtype): bool
    {
        ...
        return $subtype->isUnion() ? \RectorPrefix202511\Nette\Utils\Arrays::every($subtype->types, function ($t) {
            return $this->allows2($t instanceof self ? $t->types : [$t]);
        }) : $this->allows2($subtype->types);
    }
    private function allows2(array $subtypes): bool
    {
        return $this->isUnion() ? \RectorPrefix202511\Nette\Utils\Arrays::some($this->types, function ($t) use ($subtypes) {
    ...
    {
        return \RectorPrefix202511\Nette\Utils\Arrays::every($types, function ($type) use ($subtypes) {
            return \RectorPrefix202511\Nette\Utils\Arrays::some($subtypes, function ($subtype) use ($type) {
                return \RectorPrefix202511\Nette\Utils\Validators::isBuiltinType($type) ? strcasecmp($type, $subtype) === 0 : is_a($subtype, $type, \true);
            });
        });
    }
}
