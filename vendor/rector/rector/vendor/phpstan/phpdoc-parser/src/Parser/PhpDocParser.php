...
namespace Argtyper202511\PHPStan\PhpDocParser\Parser;

use LogicException;
use Argtyper202511\PHPStan\PhpDocParser\Ast;
...
use Argtyper202511\PHPStan\PhpDocParser\Ast\PhpDoc\Doctrine;
...
class PhpDocParser
{
    ...
    /**
     * @var \PHPStan\PhpDocParser\ParserConfig
     */
    private $config;
    /**
    ...
    private $typeParser;
    /**
     * @var \PHPStan\PhpDocParser\Parser\ConstExprParser
     */
    ...
    public function __construct(ParserConfig $config, \Argtyper202511\PHPStan\PhpDocParser\Parser\TypeParser $typeParser, \Argtyper202511\PHPStan\PhpDocParser\Parser\ConstExprParser $constantExprParser)
    {
        $this->config = $config;
    ...
    public function parse(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\PhpDocNode
    {
        ...
        } catch (\Argtyper202511\PHPStan\PhpDocParser\Parser\ParserException $e) {
            $name = '';
    ...
    /** @phpstan-impure */
    private function parseChild(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\PhpDocChildNode
    {
     ...
     * @return T
     */
    private function enrichWithAttributes(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens, Ast\Node $tag, int $startLine, int $startIndex) : Ast\Node
    {
    ...
    private function parseText(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\PhpDocTextNode
    {
        $text = '';
    ...
    private function parseOptionalDescriptionAfterDoctrineTag(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : string
    {
        $text = '';
        $endTokens = [Lexer::TOKEN_CLOSE_PHPDOC, Lexer::TOKEN_END];
        $savepoint = \false;
    ...
    public function parseTag(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\PhpDocTagNode
    {
        $tag = $tokens->currentTokenValue();
        $tokens->next();
    ...
    public function parseTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens, string $tag) : Ast\PhpDoc\PhpDocTagValueNode
    {
        $startLine = $tokens->currentTokenLine();
        $startIndex = $tokens->currentTokenIndex();
        try {
            ...
            switch ($tag) {
                ...
                case '@psalm-template-contravariant':
                    $tagValue = $this->typeParser->parseTemplateTagValue($tokens, function ($tokens) {
                        return $this->parseOptionalDescription($tokens, \true);
                    });
                    break;
                case '@extends':
    ...
    private function parseDoctrineTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens, string $tag) : Ast\PhpDoc\PhpDocTagValueNode
    {
        $startLine = $tokens->currentTokenLine();
        $startIndex = $tokens->currentTokenIndex();
     ...
     */
    private function parseDoctrineArguments(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens, bool $deep) : array
    {
        if (!$tokens->isCurrentTokenType(Lexer::TOKEN_OPEN_PARENTHESES)) {
            return [];
        }
    ...
    private function parseDoctrineArgument(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Doctrine\DoctrineArgument
    {
     ...
     */
    private function parseDoctrineArgumentValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens)
    {
        $startLine = $tokens->currentTokenLine();
        $startIndex = $tokens->currentTokenIndex();
        ...
        try {
            ...
            if ($constExpr instanceof Ast\ConstExpr\ConstExprArrayNode) {
                throw new \Argtyper202511\PHPStan\PhpDocParser\Parser\ParserException($currentTokenValue, $currentTokenType, $currentTokenOffset, Lexer::TOKEN_IDENTIFIER, null, $currentTokenLine);
            }
            return $constExpr;
    ...
    private function parseDoctrineArrayItem(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Doctrine\DoctrineArrayItem
    {
        $startLine = $tokens->currentTokenLine();
        $startIndex = $tokens->currentTokenIndex();
        try {
     ...
     */
    private function parseDoctrineArrayKey(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens)
    {
        $startLine = $tokens->currentTokenLine();
        $startIndex = $tokens->currentTokenIndex();
        if ($tokens->isCurrentTokenType(Lexer::TOKEN_INTEGER)) {
            ...
            $key = new Ast\ConstExpr\ConstExprStringNode(\Argtyper202511\PHPStan\PhpDocParser\Parser\StringUnescaper::unescapeString($tokens->currentTokenValue()), Ast\ConstExpr\ConstExprStringNode::SINGLE_QUOTED);
            $tokens->next();
            ...
            if (!$tokens->tryConsumeTokenType(Lexer::TOKEN_IDENTIFIER)) {
                $tokens->dropSavePoint();
                throw new \Argtyper202511\PHPStan\PhpDocParser\Parser\ParserException($tokens->currentTokenValue(), $tokens->currentTokenType(), $tokens->currentTokenOffset(), Lexer::TOKEN_IDENTIFIER, null, $tokens->currentTokenLine());
            }
            if (!$tokens->isCurrentTokenType(Lexer::TOKEN_DOUBLE_COLON)) {
                $tokens->dropSavePoint();
     ...
     */
    private function parseParamTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\PhpDocTagValueNode
    {
    ...
    private function parseParamImmediatelyInvokedCallableTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\ParamImmediatelyInvokedCallableTagValueNode
    {
        $parameterName = $this->parseRequiredVariableName($tokens);
        $description = $this->parseOptionalDescription($tokens, \false);
        return new Ast\PhpDoc\ParamImmediatelyInvokedCallableTagValueNode($parameterName, $description);
    }
    private function parseParamLaterInvokedCallableTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\ParamLaterInvokedCallableTagValueNode
    {
        $parameterName = $this->parseRequiredVariableName($tokens);
        $description = $this->parseOptionalDescription($tokens, \false);
        return new Ast\PhpDoc\ParamLaterInvokedCallableTagValueNode($parameterName, $description);
    }
    private function parseParamClosureThisTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\ParamClosureThisTagValueNode
    {
        $type = $this->typeParser->parse($tokens);
        $parameterName = $this->parseRequiredVariableName($tokens);
        $description = $this->parseOptionalDescription($tokens, \false);
    ...
    private function parsePureUnlessCallableIsImpureTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\PureUnlessCallableIsImpureTagValueNode
    {
        $parameterName = $this->parseRequiredVariableName($tokens);
        $description = $this->parseOptionalDescription($tokens, \false);
        return new Ast\PhpDoc\PureUnlessCallableIsImpureTagValueNode($parameterName, $description);
    }
    private function parseVarTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\VarTagValueNode
    {
        $type = $this->typeParser->parse($tokens);
    ...
    private function parseReturnTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\ReturnTagValueNode
    {
        $type = $this->typeParser->parse($tokens);
        $description = $this->parseOptionalDescription($tokens, \true);
        return new Ast\PhpDoc\ReturnTagValueNode($type, $description);
    }
    private function parseThrowsTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\ThrowsTagValueNode
    {
        $type = $this->typeParser->parse($tokens);
        $description = $this->parseOptionalDescription($tokens, \true);
        return new Ast\PhpDoc\ThrowsTagValueNode($type, $description);
    }
    private function parseMixinTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\MixinTagValueNode
    {
        $type = $this->typeParser->parse($tokens);
        $description = $this->parseOptionalDescription($tokens, \true);
        return new Ast\PhpDoc\MixinTagValueNode($type, $description);
    }
    private function parseRequireExtendsTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\RequireExtendsTagValueNode
    {
        $type = $this->typeParser->parse($tokens);
        $description = $this->parseOptionalDescription($tokens, \true);
        return new Ast\PhpDoc\RequireExtendsTagValueNode($type, $description);
    }
    private function parseRequireImplementsTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\RequireImplementsTagValueNode
    {
        $type = $this->typeParser->parse($tokens);
        $description = $this->parseOptionalDescription($tokens, \true);
        return new Ast\PhpDoc\RequireImplementsTagValueNode($type, $description);
    }
    private function parseSealedTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\SealedTagValueNode
    {
        $type = $this->typeParser->parse($tokens);
        $description = $this->parseOptionalDescription($tokens, \true);
        return new Ast\PhpDoc\SealedTagValueNode($type, $description);
    }
    private function parseDeprecatedTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\DeprecatedTagValueNode
    {
        $description = $this->parseOptionalDescription($tokens, \false);
        return new Ast\PhpDoc\DeprecatedTagValueNode($description);
    }
    private function parsePropertyTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\PropertyTagValueNode
    {
        $type = $this->typeParser->parse($tokens);
        $parameterName = $this->parseRequiredVariableName($tokens);
        $description = $this->parseOptionalDescription($tokens, \false);
    ...
    private function parseMethodTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\MethodTagValueNode
    {
    ...
    private function parseMethodTagValueParameter(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\MethodTagValueParameterNode
    {
        $startLine = $tokens->currentTokenLine();
        $startIndex = $tokens->currentTokenIndex();
    ...
    private function parseExtendsTagValue(string $tagName, \Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\PhpDocTagValueNode
    {
        $startLine = $tokens->currentTokenLine();
        $startIndex = $tokens->currentTokenIndex();
    ...
    private function parseTypeAliasTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\TypeAliasTagValueNode
    {
        ...
        try {
            ...
            if (!$tokens->isCurrentTokenType(Lexer::TOKEN_CLOSE_PHPDOC)) {
                if (!$tokens->isCurrentTokenType(Lexer::TOKEN_PHPDOC_EOL)) {
                    throw new \Argtyper202511\PHPStan\PhpDocParser\Parser\ParserException($tokens->currentTokenValue(), $tokens->currentTokenType(), $tokens->currentTokenOffset(), Lexer::TOKEN_PHPDOC_EOL, null, $tokens->currentTokenLine());
                }
            }
    ...
    private function parseTypeAliasImportTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\TypeAliasImportTagValueNode
    {
     ...
     */
    private function parseAssertTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\PhpDocTagValueNode
    {
     ...
     */
    private function parseAssertParameter(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : array
    {
    ...
    private function parseSelfOutTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\SelfOutTagValueNode
    {
        $type = $this->typeParser->parse($tokens);
        $description = $this->parseOptionalDescription($tokens, \true);
        return new Ast\PhpDoc\SelfOutTagValueNode($type, $description);
    }
    private function parseParamOutTagValue(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : Ast\PhpDoc\ParamOutTagValueNode
    {
        $type = $this->typeParser->parse($tokens);
        $parameterName = $this->parseRequiredVariableName($tokens);
        $description = $this->parseOptionalDescription($tokens, \false);
    ...
    private function parseOptionalVariableName(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : string
    {
    ...
    private function parseRequiredVariableName(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens) : string
    {
        $parameterName = $tokens->currentTokenValue();
        $tokens->consumeTokenType(Lexer::TOKEN_VARIABLE);
        return $parameterName;
     ...
     */
    private function parseOptionalDescription(\Argtyper202511\PHPStan\PhpDocParser\Parser\TokenIterator $tokens, bool $limitStartToken) : string
    {
        if ($limitStartToken) {
