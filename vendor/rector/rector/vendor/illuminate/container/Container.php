...
class Container implements ArrayAccess, ContainerContract
{
    ...
    {
        $aliases = [];
        foreach (\RectorPrefix202511\Illuminate\Container\Util::arrayWrap($concrete) as $c) {
            $aliases[] = $this->getAlias($c);
        }
        return new \RectorPrefix202511\Illuminate\Container\ContextualBindingBuilder($this, $aliases);
    }
    /**
    ...
    public function tagged($tag)
    {
        ...
        return new \RectorPrefix202511\Illuminate\Container\RewindableGenerator(function () use ($tag) {
    ...
    public function call($callback, array $parameters = [], $defaultMethod = null)
    {
        ...
        $result = \RectorPrefix202511\Illuminate\Container\BoundMethod::call($this, $callback, $parameters, $defaultMethod);
    ...
    public function get(string $id)
    {
        try {
            ...
            throw new \RectorPrefix202511\Illuminate\Container\EntryNotFoundException($id, is_int($e->getCode()) ? $e->getCode() : 0, $e);
        }
    }
    /**
     * Resolve the given type from the container.
    ...
    protected function resolveDependencies(array $dependencies)
    {
        ...
        foreach ($dependencies as $dependency) {
            ...
            $result = null;
            if (!is_null($attribute = \RectorPrefix202511\Illuminate\Container\Util::getContextualAttributeFromDependency($dependency))) {
                $result = $this->resolveFromAttribute($attribute);
            }
            // If the class is null, it means the dependency is a string or some other
            // primitive type which we can not resolve since it is not a class and
            // we will just bomb out with an error since we have no-where to go.
            $result = $result ?? is_null(\RectorPrefix202511\Illuminate\Container\Util::getParameterClassName($dependency)) ? $this->resolvePrimitive($dependency) : $this->resolveClass($dependency);
    ...
    protected function resolvePrimitive(ReflectionParameter $parameter)
    {
        if (!is_null($concrete = $this->getContextualConcrete('$' . $parameter->getName()))) {
            return \RectorPrefix202511\Illuminate\Container\Util::unwrapIfClosure($concrete, $this);
        }
    ...
    {
        try {
            return $parameter->isVariadic() ? $this->resolveVariadicClass($parameter) : $this->make(\RectorPrefix202511\Illuminate\Container\Util::getParameterClassName($parameter));
        } catch (BindingResolutionException $e) {
            if ($parameter->isDefaultValueAvailable()) {
                array_pop($this->with);
                return $parameter->getDefaultValue();
    ...
    {
        $className = \RectorPrefix202511\Illuminate\Container\Util::getParameterClassName($parameter);
