...
class XdebugHandler
{
    ...
    public function __construct(string $envPrefix)
    {
        ...
        $this->statusWriter = new \RectorPrefix202511\Composer\XdebugHandler\Status($this->envAllowXdebug, (bool) $this->debug);
    }
    /**
    ...
    {
        $this->notify(\RectorPrefix202511\Composer\XdebugHandler\Status::CHECK, self::$xdebugVersion . '|' . self::$xdebugMode);
        $envArgs = explode('|', (string) getenv($this->envAllowXdebug));
        if (!(bool) $envArgs[0] && $this->requiresRestart(self::$xdebugActive)) {
            // Restart required
            $this->notify(\RectorPrefix202511\Composer\XdebugHandler\Status::RESTART);
        ...
        if (self::RESTART_ID === $envArgs[0] && count($envArgs) === 5) {
            ...
            $this->notify(\RectorPrefix202511\Composer\XdebugHandler\Status::RESTARTED);
            \RectorPrefix202511\Composer\XdebugHandler\Process::setEnv($this->envAllowXdebug);
            self::$inRestart = \true;
        ...
        $this->notify(\RectorPrefix202511\Composer\XdebugHandler\Status::NORESTART);
    ...
    private function doRestart(array $command): void
    {
        if (\PHP_VERSION_ID >= 70400) {
            ...
            $cmd = \RectorPrefix202511\Composer\XdebugHandler\Process::escapeShellCommand($command);
        ...
        $this->tryEnableSignals();
        $this->notify(\RectorPrefix202511\Composer\XdebugHandler\Status::RESTARTING, $displayCmd);
        ...
        if (!isset($exitCode)) {
            ...
            $this->notify(\RectorPrefix202511\Composer\XdebugHandler\Status::ERROR, 'Unable to restart process');
            $exitCode = -1;
        } else {
            $this->notify(\RectorPrefix202511\Composer\XdebugHandler\Status::INFO, 'Restarted process exited ' . $exitCode);
        }
        if ($this->debug === '2') {
            $this->notify(\RectorPrefix202511\Composer\XdebugHandler\Status::INFO, 'Temp ini saved: ' . $this->tmpIni);
        } else {
    ...
    {
        if (!$this->cli) {
            $this->notify(\RectorPrefix202511\Composer\XdebugHandler\Status::ERROR, 'Unsupported SAPI: ' . \PHP_SAPI);
            return null;
        }
        if (($argv = $this->checkServerArgv()) === null) {
            $this->notify(\RectorPrefix202511\Composer\XdebugHandler\Status::ERROR, '$_SERVER[argv] is not as expected');
            return null;
        }
        if (!$this->checkConfiguration($info)) {
            $this->notify(\RectorPrefix202511\Composer\XdebugHandler\Status::ERROR, $info);
            return null;
        }
        $mainScript = (string) $this->script;
        if (!$this->checkMainScript($mainScript, $argv)) {
            $this->notify(\RectorPrefix202511\Composer\XdebugHandler\Status::ERROR, 'Unable to access main script: ' . $mainScript);
            return null;
        }
        $tmpDir = sys_get_temp_dir();
        $iniError = 'Unable to create temp ini file at: ' . $tmpDir;
        if (($tmpfile = @tempnam($tmpDir, '')) === \false) {
            $this->notify(\RectorPrefix202511\Composer\XdebugHandler\Status::ERROR, $iniError);
            return null;
        }
        $error = null;
        ...
        if (!$this->writeTmpIni($tmpfile, $iniFiles, $error)) {
            $this->notify(\RectorPrefix202511\Composer\XdebugHandler\Status::ERROR, $error ?? $iniError);
            @unlink($tmpfile);
            return null;
        }
        if (!$this->setEnvironment($scannedInis, $iniFiles, $tmpfile)) {
            $this->notify(\RectorPrefix202511\Composer\XdebugHandler\Status::ERROR, 'Unable to set environment variables');
            @unlink($tmpfile);
            return null;
        }
        $this->tmpIni = $tmpfile;
    ...
    private function setEnvRestartSettings(array $envArgs): void
    {
        ...
        \RectorPrefix202511\Composer\XdebugHandler\Process::setEnv(self::RESTART_SETTINGS, implode('|', $settings));
    }
    /**
    ...
    private function syncSettings(array $settings): void
    {
        if (\false === getenv($this->envOriginalInis)) {
            ...
            \RectorPrefix202511\Composer\XdebugHandler\Process::setEnv($this->envOriginalInis, implode(\PATH_SEPARATOR, $settings['inis']));
        }
        self::$skipped = $settings['skipped'];
        $this->notify(\RectorPrefix202511\Composer\XdebugHandler\Status::INFO, 'Process called with existing restart settings');
    }
    /**
