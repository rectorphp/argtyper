...
CODE_SAMPLE
    ...
    public function refactor(Node $node): ?Node
    {
        ...
        $filesystemLoaderNew->class = new FullyQualified('Argtyper202601\Twig\Loader\FilesystemLoader');
    ...
    private function resolveFileSystemLoaderNew(Node $stmtsAware): ?New_
    {
        foreach ((array) $stmtsAware->stmts as $stmt) {
            ...
            $new = $assign->expr;
            if (!$this->isObjectType($new, new ObjectType('Argtyper202601\Symfony\Bundle\TwigBundle\Loader\FilesystemLoader'))) {
                continue;
            }
            return $new;
        }
