...
CODE_SAMPLE
    ...
    private function shouldSkip(New_ $new): bool
    {
        ...
        if (!$this->isName($new->class, 'Argtyper202601\Symfony\Component\PropertyAccess\PropertyAccessor')) {
            return \true;
        }
        if (!isset($new->args[0])) {
            return \true;
    ...
    {
        $magicGetClassConstFetch = $this->nodeFactory->createClassConstFetch('Argtyper202601\Symfony\Component\PropertyAccess\PropertyAccessor', 'MAGIC_GET');
        $magicSetClassConstFetch = $this->nodeFactory->createClassConstFetch('Argtyper202601\Symfony\Component\PropertyAccess\PropertyAccessor', 'MAGIC_SET');
        if (!$currentValue) {
            return new BitwiseOr($magicGetClassConstFetch, $magicSetClassConstFetch);
        }
        $magicCallClassConstFetch = $this->nodeFactory->createClassConstFetch('Argtyper202601\Symfony\Component\PropertyAccess\PropertyAccessor', 'MAGIC_CALL');
