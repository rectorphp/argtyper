...
CODE_SAMPLE
    ...
    private function isClassAndMethodMatch(Class_ $class, ClassMethod $classMethod): bool
    {
        if ($this->isName($classMethod->name, 'getParent')) {
            return $this->isObjectType($class, new ObjectType('Argtyper202601\Symfony\Component\Form\AbstractType'));
        }
        if ($this->isName($classMethod->name, 'getExtendedType')) {
            return $this->isObjectType($class, new ObjectType('Argtyper202601\Symfony\Component\Form\AbstractTypeExtension'));
        }
        return \false;
    }
