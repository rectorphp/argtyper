...
namespace Argtyper202511\Rector\Doctrine\Orm30\Rector\MethodCall;
...
CODE_SAMPLE
    ...
    public function changeArrayToCollection(array $stmts, Variable $variable): bool
    {
        ...
        foreach ($stmts as $stmt) {
            ...
            if ($stmt->expr instanceof Assign && $stmt->expr->expr instanceof Array_ && $stmt->expr->var instanceof Variable && $stmt->expr->var->name === $variable->name) {
                $newCollection = new New_(new FullyQualified('Argtyper202511\Doctrine\Common\Collections\ArrayCollection'));
            ...
            if ($stmt->expr instanceof Assign && $stmt->expr->var instanceof ArrayDimFetch && $stmt->expr->var->dim instanceof Expr && $stmt->expr->var->var instanceof Variable && $stmt->expr->var->var->name === $variable->name) {
                $newParameter = new New_(new FullyQualified('Argtyper202511\Doctrine\ORM\Query\Parameter'));
    ...
    private function getAffectedVariables(ClassMethod $classMethod): iterable
    {
        ...
        foreach ($statements as $statement) {
            if ($statement instanceof Expression && $statement->expr instanceof MethodCall && $statement->expr->name instanceof Identifier && $statement->expr->name->name === 'setParameters' && count($statement->expr->args) === 1 && $statement->expr->args[0] instanceof Arg && $statement->expr->args[0]->value instanceof Variable) {
                ...
                if (!$varType instanceof ObjectType || !$varType->isInstanceOf('Argtyper202511\Doctrine\ORM\QueryBuilder')->yes()) {
                    continue;
                }
    ...
    private function refactorMethodCall(MethodCall $methodCall): ?\Argtyper202511\PhpParser\Node
    {
        $varType = $this->nodeTypeResolver->getType($methodCall->var);
        if (!$varType instanceof ObjectType) {
            return null;
        }
        if (!$varType->isInstanceOf('Argtyper202511\Doctrine\ORM\QueryBuilder')->yes()) {
            return null;
        }
        ...
        if ($currentArgType instanceof ObjectType && $currentArgType->isInstanceOf('Argtyper202511\Doctrine\Common\Collections\ArrayCollection')->yes() && $currentArg instanceof New_ && count($currentArg->args) === 1 && $currentArg->args[0] instanceof Arg) {
    ...
    private function convertArrayToParameters(Array_ $array): array
    {
        ...
        foreach ($array->items as $index => $value) {
            ...
            if (!$arrayValueType instanceof ObjectType || !$arrayValueType->isInstanceOf('Argtyper202511\Doctrine\ORM\Query\Parameter')->yes()) {
                $newParameter = new New_(new FullyQualified('Argtyper202511\Doctrine\ORM\Query\Parameter'));
