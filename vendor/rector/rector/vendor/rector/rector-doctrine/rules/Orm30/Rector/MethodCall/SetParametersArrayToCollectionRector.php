...
CODE_SAMPLE
    ...
    private function refactorMethodCall(MethodCall $methodCall): ?\PhpParser\Node
    {
        ...
        if (!$varType->isInstanceOf('Argtyper202601\Doctrine\ORM\QueryBuilder')->yes()) {
            return null;
        }
        ...
        if ($currentArgType instanceof ObjectType && $currentArgType->isInstanceOf('Argtyper202601\Doctrine\Common\Collections\ArrayCollection')->yes() && $currentArg instanceof New_ && count($currentArg->args) === 1 && $currentArg->args[0] instanceof Arg) {
        ...
        $new = new New_(new FullyQualified('Argtyper202601\Doctrine\Common\Collections\ArrayCollection'));
    ...
    private function convertArrayToParameters(Array_ $array): array
    {
        ...
        foreach ($array->items as $index => $value) {
            ...
            if (!$arrayValueType instanceof ObjectType || !$arrayValueType->isInstanceOf('Argtyper202601\Doctrine\ORM\Query\Parameter')->yes()) {
                $newParameter = new New_(new FullyQualified('Argtyper202601\Doctrine\ORM\Query\Parameter'));
