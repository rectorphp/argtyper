...
final class CastDoctrineExprToStringRector extends AbstractRector
{
     ...
     * @var array<string>
     */
    private $targetMethods = ['like', 'notLike', 'eq', 'neq', 'lt', 'lte', 'gt', 'gte', 'between', 'in', 'notIn', 'isMemberOf', 'isInstanceOf'];
    /**
     * @var array<string>
     */
    private $exprFuncMethods = ['lower', 'upper', 'length', 'trim', 'avg', 'max', 'min', 'count', 'countDistinct', 'exists', 'all', 'some', 'any', 'not', 'abs', 'sqrt'];
...
CODE_SAMPLE
    ...
    public function refactor(Node $node): ?Node
    {
        ...
        if (!$this->isObjectType($node->var, new ObjectType('Argtyper202511\Doctrine\ORM\Query\Expr'))) {
            return null;
        }
        ...
        foreach ($node->args as $arg) {
            ...
            if ($arg->value instanceof MethodCall && $this->isObjectType($arg->value->var, new ObjectType('Argtyper202511\Doctrine\ORM\Query\Expr')) && in_array($this->getName($arg->value->name), $this->exprFuncMethods, \true)) {
