...
namespace Argtyper202511\RectorPrefix202511\React\EventLoop;

use Argtyper202511\RectorPrefix202511\React\EventLoop\Tick\FutureTickQueue;
use Argtyper202511\RectorPrefix202511\React\EventLoop\Timer\Timer;
...
final class ExtUvLoop implements LoopInterface
{
    ...
    {
        if (!\function_exists('uv_loop_new') && !\function_exists('Argtyper202511\\RectorPrefix202511\\uv_loop_new')) {
    ...
    public function addSignal($signal, $listener)
    {
        ...
        if (!isset($this->signalEvents[$signal])) {
            ...
            $this->signalEvents[$signal] = \Argtyper202511\uv_signal_init($this->uv);
            \Argtyper202511\uv_signal_start($this->signalEvents[$signal], function () use($signals, $signal) {
                $signals->call($signal);
            }, $signal);
        }
    }
    ...
    {
        if (!isset($this->streamEvents[(int) $stream])) {
            $this->streamEvents[(int) $stream] = \Argtyper202511\RectorPrefix202511\uv_poll_init_socket($this->uv, $stream);
        }
