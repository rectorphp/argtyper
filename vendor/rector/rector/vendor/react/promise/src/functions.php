 ...
 */
function resolve($promiseOrValue): \RectorPrefix202512\React\Promise\PromiseInterface
{
    if ($promiseOrValue instanceof \RectorPrefix202512\React\Promise\PromiseInterface) {
    ...
    if (\is_object($promiseOrValue) && \method_exists($promiseOrValue, 'then')) {
        ...
        /** @var Promise<T> */
        return new \RectorPrefix202512\React\Promise\Promise(function (callable $resolve, callable $reject) use ($promiseOrValue): void {
 ...
 */
function reject(\Throwable $reason): \RectorPrefix202512\React\Promise\PromiseInterface
{
 ...
 */
function all(iterable $promisesOrValues): \RectorPrefix202512\React\Promise\PromiseInterface
{
    $cancellationQueue = new \RectorPrefix202512\React\Promise\Internal\CancellationQueue();
    /** @var Promise<array<T>> */
    return new \RectorPrefix202512\React\Promise\Promise(function (callable $resolve, callable $reject) use ($promisesOrValues, $cancellationQueue): void {
        $toResolve = 0;
        /** @var bool */
        $continue = \true;
        $values = [];
 ...
 * @return PromiseInterface<T>
 */
function race(iterable $promisesOrValues): \RectorPrefix202512\React\Promise\PromiseInterface
{
    $cancellationQueue = new \RectorPrefix202512\React\Promise\Internal\CancellationQueue();
    /** @var Promise<T> */
    return new \RectorPrefix202512\React\Promise\Promise(function (callable $resolve, callable $reject) use ($promisesOrValues, $cancellationQueue): void {
 ...
 * @return PromiseInterface<T>
 */
function any(iterable $promisesOrValues): \RectorPrefix202512\React\Promise\PromiseInterface
{
    $cancellationQueue = new \RectorPrefix202512\React\Promise\Internal\CancellationQueue();
    /** @var Promise<T> */
    return new \RectorPrefix202512\React\Promise\Promise(function (callable $resolve, callable $reject) use ($promisesOrValues, $cancellationQueue): void {
        ...
        if ($toReject === 0 && !$reasons) {
            $reject(new \RectorPrefix202512\React\Promise\Exception\LengthException('Must contain at least 1 item but contains only 0 items.'));
        } elseif ($toReject === 0) {
            $reject(new CompositeException($reasons, 'All promises rejected.'));
        }
    }, $cancellationQueue);
