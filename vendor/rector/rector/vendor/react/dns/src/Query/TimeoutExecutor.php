...
final class TimeoutExecutor implements \RectorPrefix202511\React\Dns\Query\ExecutorInterface
{
    private $executor;
    private $loop;
    private $timeout;
     ...
     */
    public function __construct(\RectorPrefix202511\React\Dns\Query\ExecutorInterface $executor, $timeout, $loop = null)
    {
    ...
    public function query(\RectorPrefix202511\React\Dns\Query\Query $query)
    {
        ...
        return new Promise(function ($resolve, $reject) use ($loop, $time, $promise, $query) {
            ...
            $timer = $loop->addTimer($time, function () use ($time, &$promise, $reject, $query) {
                $reject(new \RectorPrefix202511\React\Dns\Query\TimeoutException('DNS query for ' . $query->describe() . ' timed out'));
