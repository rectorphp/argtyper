 ...
 */
class TcpTransportExecutor implements \RectorPrefix202512\React\Dns\Query\ExecutorInterface
{
    private $nameserver;
    private $loop;
    private $parser;
    ...
    public function query(\RectorPrefix202512\React\Dns\Query\Query $query)
    {
        ...
        $deferred = new Deferred(function () use ($that, &$names, $request) {
            ...
            $that->checkIdle();
            throw new \RectorPrefix202512\React\Dns\Query\CancellationException('DNS query for ' . $name . ' has been cancelled');
        });
