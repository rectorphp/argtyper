 ...
 */
final class UdpTransportExecutor implements \RectorPrefix202511\React\Dns\Query\ExecutorInterface
{
    private $nameserver;
    private $loop;
    private $parser;
    ...
    public function query(\RectorPrefix202511\React\Dns\Query\Query $query)
    {
        ...
        $deferred = new Deferred(function () use ($loop, $socket, $query) {
            ...
            \fclose($socket);
            throw new \RectorPrefix202511\React\Dns\Query\CancellationException('DNS query for ' . $query->describe() . ' has been cancelled');
        });
