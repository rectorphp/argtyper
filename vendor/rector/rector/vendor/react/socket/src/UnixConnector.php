 ...
 * async behavior.
 */
final class UnixConnector implements \RectorPrefix202511\React\Socket\ConnectorInterface
{
    private $loop;
    /**
    ...
    public function connect($path)
    {
        ...
        if (!$resource) {
            return Promise\reject(new \RuntimeException('Unable to connect to unix domain socket "' . $path . '": ' . $errstr . \RectorPrefix202511\React\Socket\SocketServer::errconst($errno), $errno));
        }
        $connection = new \RectorPrefix202511\React\Socket\Connection($resource, $this->loop);
        $connection->unix = \true;
