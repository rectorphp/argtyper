...
final class SecureConnector implements \RectorPrefix202511\React\Socket\ConnectorInterface
{
    private $connector;
     ...
     * @param array $context
     */
    public function __construct(\RectorPrefix202511\React\Socket\ConnectorInterface $connector, $loop = null, array $context = array())
    {
        ...
        $this->streamEncryption = new \RectorPrefix202511\React\Socket\StreamEncryption($loop ?: Loop::get(), \false);
        $this->context = $context;
    }
    public function connect($uri)
    {
        if (!\function_exists('stream_socket_enable_crypto') && !\function_exists('Argtyper202511\stream_socket_enable_crypto') && !\function_exists('RectorPrefix202511\stream_socket_enable_crypto')) {
        ...
        $promise = $this->connector->connect(\str_replace('tls://', '', $uri))->then(function (\RectorPrefix202511\React\Socket\ConnectionInterface $connection) use ($context, $encryption, $uri, &$promise, &$connected) {
            // (unencrypted) TCP/IP connection succeeded
            $connected = \true;
            if (!$connection instanceof \RectorPrefix202511\React\Socket\Connection) {
                $connection->close();
