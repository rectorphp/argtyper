...
final class SocketServer extends EventEmitter implements \RectorPrefix202512\React\Socket\ServerInterface
{
    private $server;
    /**
    ...
    public function __construct($uri, array $context = array(), $loop = null)
    {
        ...
        if ($scheme === 'unix') {
            $server = new \RectorPrefix202512\React\Socket\UnixServer($uri, $loop, $context['unix']);
        } elseif ($scheme === 'php') {
            $server = new \RectorPrefix202512\React\Socket\FdServer($uri, $loop);
        } else {
            ...
            $server = new \RectorPrefix202512\React\Socket\TcpServer(str_replace('tls://', '', $uri), $loop, $context['tcp']);
            if ($scheme === 'tls') {
                $server = new \RectorPrefix202512\React\Socket\SecureServer($server, $loop, $context['tls']);
            }
        }
        $this->server = $server;
        $that = $this;
        $server->on('connection', function (\RectorPrefix202512\React\Socket\ConnectionInterface $conn) use ($that) {
    ...
    public static function accept($socket)
    {
        ...
        \set_error_handler(function ($_, $error) use (&$errno, &$errstr) {
            ...
            $errno = \RectorPrefix202512\React\Socket\SocketServer::errno($errstr);
        });
