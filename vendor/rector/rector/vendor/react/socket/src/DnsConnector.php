...
final class DnsConnector implements \RectorPrefix202511\React\Socket\ConnectorInterface
{
    private $connector;
    private $resolver;
    public function __construct(\RectorPrefix202511\React\Socket\ConnectorInterface $connector, ResolverInterface $resolver)
    {
    ...
    public function connect($uri)
    {
        ...
        return new Promise\Promise(function ($resolve, $reject) use (&$promise, &$resolved, $uri, $connector, $host, $parts) {
            ...
            $promise->then(function ($ip) use (&$promise, &$resolved, $uri, $connector, $host, $parts) {
                $resolved = $ip;
                return $promise = $connector->connect(\RectorPrefix202511\React\Socket\Connector::uri($parts, $host, $ip))->then(null, function (\Exception $e) use ($uri) {
