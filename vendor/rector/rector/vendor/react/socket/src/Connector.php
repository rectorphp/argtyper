...
use RectorPrefix202512\React\Dns\Config\Config as DnsConfig;
use RectorPrefix202512\React\Dns\Resolver\Factory as DnsFactory;
use RectorPrefix202512\React\Dns\Resolver\ResolverInterface;
 ...
 */
final class Connector implements \RectorPrefix202512\React\Socket\ConnectorInterface
{
    ...
    public function __construct($context = array(), $loop = null)
    {
        ...
        if ($context['tcp'] instanceof \RectorPrefix202512\React\Socket\ConnectorInterface) {
            $tcp = $context['tcp'];
        } else {
            $tcp = new \RectorPrefix202512\React\Socket\TcpConnector($loop, \is_array($context['tcp']) ? $context['tcp'] : array());
        }
        if ($context['dns'] !== \false) {
            ...
            if ($context['happy_eyeballs'] === \true) {
                $tcp = new \RectorPrefix202512\React\Socket\HappyEyeBallsConnector($loop, $tcp, $resolver);
            } else {
                $tcp = new \RectorPrefix202512\React\Socket\DnsConnector($tcp, $resolver);
            }
        }
        if ($context['tcp'] !== \false) {
            $context['tcp'] = $tcp;
            if ($context['timeout'] !== \false) {
                $context['tcp'] = new \RectorPrefix202512\React\Socket\TimeoutConnector($context['tcp'], $context['timeout'], $loop);
            }
            $this->connectors['tcp'] = $context['tcp'];
        }
        if ($context['tls'] !== \false) {
            if (!$context['tls'] instanceof \RectorPrefix202512\React\Socket\ConnectorInterface) {
                $context['tls'] = new \RectorPrefix202512\React\Socket\SecureConnector($tcp, $loop, \is_array($context['tls']) ? $context['tls'] : array());
            }
            if ($context['timeout'] !== \false) {
                $context['tls'] = new \RectorPrefix202512\React\Socket\TimeoutConnector($context['tls'], $context['timeout'], $loop);
            }
            $this->connectors['tls'] = $context['tls'];
        }
        if ($context['unix'] !== \false) {
            if (!$context['unix'] instanceof \RectorPrefix202512\React\Socket\ConnectorInterface) {
                $context['unix'] = new \RectorPrefix202512\React\Socket\UnixConnector($loop);
            }
    ...
    public function connect($uri)
    {
        ...
        if (!isset($this->connectors[$scheme])) {
            return \RectorPrefix202512\React\Promise\reject(new \RuntimeException('No connector available for URI scheme "' . $scheme . '" (EINVAL)', \defined('SOCKET_EINVAL') ? \SOCKET_EINVAL : (\defined('PCNTL_EINVAL') ? \PCNTL_EINVAL : 22)));
        }
