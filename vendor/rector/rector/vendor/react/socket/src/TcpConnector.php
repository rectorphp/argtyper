...
final class TcpConnector implements \RectorPrefix202511\React\Socket\ConnectorInterface
{
    private $loop;
    private $context;
    /**
    ...
    public function connect($uri)
    {
        ...
        if (\false === $stream) {
            return Promise\reject(new \RuntimeException('Connection to ' . $uri . ' failed: ' . $errstr . \RectorPrefix202511\React\Socket\SocketServer::errconst($errno), $errno));
        }
        // wait for connection
        $loop = $this->loop;
        return new Promise\Promise(function ($resolve, $reject) use ($loop, $stream, $uri) {
            $loop->addWriteStream($stream, function ($stream) use ($loop, $resolve, $reject, $uri) {
                ...
                if (\false === \stream_socket_get_name($stream, \true)) {
                    ...
                    // @codeCoverageIgnoreEnd
                    \fclose($stream);
                    $reject(new \RuntimeException('Connection to ' . $uri . ' failed: ' . $errstr . \RectorPrefix202511\React\Socket\SocketServer::errconst($errno), $errno));
                } else {
                    $resolve(new \RectorPrefix202511\React\Socket\Connection($stream, $loop));
                }
            });
