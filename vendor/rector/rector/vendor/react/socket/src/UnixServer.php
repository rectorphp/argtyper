 ...
 */
final class UnixServer extends EventEmitter implements \RectorPrefix202511\React\Socket\ServerInterface
{
    private $master;
    private $loop;
    ...
    public function __construct($path, $loop = null, array $context = array())
    {
        ...
        if (\false === $this->master) {
            throw new \RuntimeException('Failed to listen on Unix domain socket "' . $path . '": ' . $errstr . \RectorPrefix202511\React\Socket\SocketServer::errconst($errno), $errno);
        }
    ...
    {
        $connection = new \RectorPrefix202511\React\Socket\Connection($socket, $this->loop);
        $connection->unix = \true;
