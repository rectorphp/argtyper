...
use Argtyper202511\PhpParser\NodeVisitor\CommentAnnotatingVisitor;
abstract class ParserAbstract implements \Argtyper202511\PhpParser\Parser
{
     ...
     *                               on version.
     */
    public function __construct(\Argtyper202511\PhpParser\Lexer $lexer, ?\Argtyper202511\PhpParser\PhpVersion $phpVersion = null)
    {
        $this->lexer = $lexer;
        $this->phpVersion = $phpVersion ?? \Argtyper202511\PhpParser\PhpVersion::getNewestSupported();
     ...
     */
    public function parse(string $code, ?\Argtyper202511\PhpParser\ErrorHandler $errorHandler = null) : ?array
    {
        $this->errorHandler = $errorHandler ?: new \Argtyper202511\PhpParser\ErrorHandler\Throwing();
        ...
        foreach ($this->createdArrays as $node) {
            foreach ($node->items as $item) {
                if ($item->value instanceof Expr\Error) {
                    $this->errorHandler->handleError(new \Argtyper202511\PhpParser\Error('Cannot use empty array elements in arrays', $item->getAttributes()));
                }
            }
        }
        ...
        if ($result !== null) {
            $traverser = new \Argtyper202511\PhpParser\NodeTraverser(new CommentAnnotatingVisitor($this->tokens));
        ...
        for (;;) {
            ...
            for (;;) {
                ...
                if ($rule !== $this->unexpectedTokenRule) {
                    ...
                    } catch (\Argtyper202511\PhpParser\Error $e) {
                        if (-1 === $e->getStartLine()) {
                    ...
                    switch ($this->errorState) {
                        case 0:
                            ...
                            $this->emitError(new \Argtyper202511\PhpParser\Error($msg, $this->getAttributesForToken($this->tokenPos)));
    ...
    protected function emitError(\Argtyper202511\PhpParser\Error $error) : void
    {
    ...
    */
    ...
    protected function handleNamespaces(array $stmts) : array
    {
        ...
        if ('brace' === $style) {
            ...
            foreach ($stmts as $stmt) {
                if ($stmt instanceof \Argtyper202511\PhpParser\Node\Stmt\Namespace_) {
                    $afterFirstNamespace = \true;
                } elseif (!$stmt instanceof \Argtyper202511\PhpParser\Node\Stmt\HaltCompiler && !$stmt instanceof \Argtyper202511\PhpParser\Node\Stmt\Nop && $afterFirstNamespace && !$hasErrored) {
                    $this->emitError(new \Argtyper202511\PhpParser\Error('No code may exist outside of namespace {}', $stmt->getAttributes()));
                    $hasErrored = \true;
            ...
            foreach ($stmts as $stmt) {
                if ($stmt instanceof \Argtyper202511\PhpParser\Node\Stmt\Namespace_) {
                    ...
                    $lastNs = $stmt;
                } elseif ($stmt instanceof \Argtyper202511\PhpParser\Node\Stmt\HaltCompiler) {
    ...
    private function fixupNamespaceAttributes(\Argtyper202511\PhpParser\Node\Stmt\Namespace_ $stmt) : void
    {
    ...
    private function getNamespacingStyle(array $stmts) : ?string
    {
        ...
        foreach ($stmts as $i => $stmt) {
            if ($stmt instanceof \Argtyper202511\PhpParser\Node\Stmt\Namespace_) {
                ...
                if (null === $style) {
                    ...
                    if ($hasNotAllowedStmts) {
                        $this->emitError(new \Argtyper202511\PhpParser\Error('Namespace declaration statement has to be the very first statement in the script', $this->getNamespaceErrorAttributes($stmt)));
                    }
                } elseif ($style !== $currentStyle) {
                    $this->emitError(new \Argtyper202511\PhpParser\Error('Cannot mix bracketed namespace declarations with unbracketed namespace declarations', $this->getNamespaceErrorAttributes($stmt)));
            ...
            if ($stmt instanceof \Argtyper202511\PhpParser\Node\Stmt\Declare_ || $stmt instanceof \Argtyper202511\PhpParser\Node\Stmt\HaltCompiler || $stmt instanceof \Argtyper202511\PhpParser\Node\Stmt\Nop) {
                continue;
            }
            /* There may be a hashbang line at the very start of the file */
            if ($i === 0 && $stmt instanceof \Argtyper202511\PhpParser\Node\Stmt\InlineHTML && \preg_match('/\\A#!.*\\r?\\n\\z/', $stmt->value)) {
                continue;
            }
    ...
    protected function handleBuiltinTypes(Name $name)
    {
        ...
        return new \Argtyper202511\PhpParser\Node\Identifier($lowerName, $name->getAttributes());
    }
    /**
    ...
    protected function parseLNumber(string $str, array $attributes, bool $allowInvalidOctal = \false) : Int_
    {
        ...
        } catch (\Argtyper202511\PhpParser\Error $error) {
    ...
    protected function stripIndentation(string $string, int $indentLen, string $indentChar, bool $newlineAtStart, bool $newlineAtEnd, array $attributes) : string
    {
        ...
        return \preg_replace_callback($regex, function ($matches) use($indentLen, $indentChar, $attributes) {
            ...
            if (\false !== \strpos($prefix, $indentChar === " " ? "\t" : " ")) {
                $this->emitError(new \Argtyper202511\PhpParser\Error('Invalid indentation - tabs and spaces cannot be mixed', $attributes));
            } elseif (\strlen($prefix) < $indentLen && !isset($matches[2])) {
                $this->emitError(new \Argtyper202511\PhpParser\Error('Invalid body indentation level ' . '(expecting an indentation level of at least ' . $indentLen . ')', $attributes));
            }
    ...
    protected function parseDocString(string $startToken, $contents, string $endToken, array $attributes, array $endTokenAttributes, bool $parseUnicodeEscape) : Expr
    {
        ...
        if ($indentHasSpaces && $indentHasTabs) {
            $this->emitError(new \Argtyper202511\PhpParser\Error('Invalid indentation - tabs and spaces cannot be mixed', $endTokenAttributes));
        ...
        if (\is_string($contents)) {
            ...
            if (!$contents[0] instanceof \Argtyper202511\PhpParser\Node\InterpolatedStringPart) {
                // If there is no leading encapsed string part, pretend there is an empty one
                $this->stripIndentation('', $indentLen, $indentChar, \true, \false, $contents[0]->getAttributes());
            }
            $newContents = [];
            foreach ($contents as $i => $part) {
                if ($part instanceof \Argtyper202511\PhpParser\Node\InterpolatedStringPart) {
    ...
    protected function createCommentFromToken(\Argtyper202511\PhpParser\Token $token, int $tokenPos) : \Argtyper202511\PhpParser\Comment
    {
        \assert($token->id === \T_COMMENT || $token->id == \T_DOC_COMMENT);
        return \T_DOC_COMMENT === $token->id ? new \Argtyper202511\PhpParser\Comment\Doc($token->text, $token->line, $token->pos, $tokenPos, $token->getEndLine(), $token->getEndPos() - 1, $tokenPos) : new \Argtyper202511\PhpParser\Comment($token->text, $token->line, $token->pos, $tokenPos, $token->getEndLine(), $token->getEndPos() - 1, $tokenPos);
    }
    /**
     * Get last comment before the given token position, if any
     */
    protected function getCommentBeforeToken(int $tokenPos) : ?\Argtyper202511\PhpParser\Comment
    {
        while (--$tokenPos >= 0) {
            $token = $this->tokens[$tokenPos];
    ...
    protected function fixupArrayDestructuring(Array_ $node) : Expr\List_
    {
        ...
        return new Expr\List_(\array_map(function (\Argtyper202511\PhpParser\Node\ArrayItem $item) {
            if ($item->value instanceof Expr\Error) {
                // We used Error as a placeholder for empty elements, which are legal for destructuring.
                return null;
            }
            if ($item->value instanceof Array_) {
                return new \Argtyper202511\PhpParser\Node\ArrayItem($this->fixupArrayDestructuring($item->value), $item->key, $item->byRef, $item->getAttributes());
            }
            return $item;
    ...
    {
        try {
            \Argtyper202511\PhpParser\Modifiers::verifyClassModifier($a, $b);
        } catch (\Argtyper202511\PhpParser\Error $error) {
    ...
    protected function checkModifier(int $a, int $b, int $modifierPos) : void
    {
        ...
        try {
            \Argtyper202511\PhpParser\Modifiers::verifyModifier($a, $b);
        } catch (\Argtyper202511\PhpParser\Error $error) {
            $error->setAttributes($this->getAttributesAt($modifierPos));
            $this->emitError($error);
        }
    ...
    protected function checkParam(Param $node) : void
    {
        if ($node->variadic && null !== $node->default) {
            $this->emitError(new \Argtyper202511\PhpParser\Error('Variadic parameter cannot have a default value', $node->default->getAttributes()));
        }
    }
    protected function checkTryCatch(TryCatch $node) : void
    {
        if (empty($node->catches) && null === $node->finally) {
            $this->emitError(new \Argtyper202511\PhpParser\Error('Cannot use try without catch or finally', $node->getAttributes()));
        }
    }
        ...
        if (null !== $node->stmts) {
            foreach ($node->stmts as $stmt) {
                if ($stmt instanceof Namespace_) {
                    $this->emitError(new \Argtyper202511\PhpParser\Error('Namespace declarations cannot be nested', $stmt->getAttributes()));
                }
            }
        }
    }
    private function checkClassName(?Identifier $name, int $namePos) : void
    {
        if (null !== $name && $name->isSpecialClassName()) {
            $this->emitError(new \Argtyper202511\PhpParser\Error(\sprintf('Cannot use \'%s\' as class name as it is reserved', $name), $this->getAttributesAt($namePos)));
        }
    }
    ...
    private function checkImplementedInterfaces(array $interfaces) : void
    {
        foreach ($interfaces as $interface) {
            if ($interface->isSpecialClassName()) {
                $this->emitError(new \Argtyper202511\PhpParser\Error(\sprintf('Cannot use \'%s\' as interface name as it is reserved', $interface), $interface->getAttributes()));
            }
        }
    }
    protected function checkClass(Class_ $node, int $namePos) : void
    {
        ...
        if ($node->extends && $node->extends->isSpecialClassName()) {
            $this->emitError(new \Argtyper202511\PhpParser\Error(\sprintf('Cannot use \'%s\' as class name as it is reserved', $node->extends), $node->extends->getAttributes()));
        }
    ...
    {
        if ($node->flags & \Argtyper202511\PhpParser\Modifiers::STATIC) {
            switch ($node->name->toLowerString()) {
                case '__construct':
                    $this->emitError(new \Argtyper202511\PhpParser\Error(\sprintf('Constructor %s() cannot be static', $node->name), $this->getAttributesAt($modifierPos)));
                    break;
                case '__destruct':
                    $this->emitError(new \Argtyper202511\PhpParser\Error(\sprintf('Destructor %s() cannot be static', $node->name), $this->getAttributesAt($modifierPos)));
                    break;
                case '__clone':
                    $this->emitError(new \Argtyper202511\PhpParser\Error(\sprintf('Clone method %s() cannot be static', $node->name), $this->getAttributesAt($modifierPos)));
                    break;
            }
        }
        if ($node->flags & \Argtyper202511\PhpParser\Modifiers::READONLY) {
            $this->emitError(new \Argtyper202511\PhpParser\Error(\sprintf('Method %s() cannot be readonly', $node->name), $this->getAttributesAt($modifierPos)));
        }
    }
    protected function checkClassConst(ClassConst $node, int $modifierPos) : void
    {
        foreach ([\Argtyper202511\PhpParser\Modifiers::STATIC, \Argtyper202511\PhpParser\Modifiers::ABSTRACT, \Argtyper202511\PhpParser\Modifiers::READONLY] as $modifier) {
            if ($node->flags & $modifier) {
                $this->emitError(new \Argtyper202511\PhpParser\Error("Cannot use '" . \Argtyper202511\PhpParser\Modifiers::toString($modifier) . "' as constant modifier", $this->getAttributesAt($modifierPos)));
            }
        }
    }
    protected function checkUseUse(UseItem $node, int $namePos) : void
    {
        if ($node->alias && $node->alias->isSpecialClassName()) {
            $this->emitError(new \Argtyper202511\PhpParser\Error(\sprintf('Cannot use %s as %s because \'%2$s\' is a special class name', $node->name, $node->alias), $this->getAttributesAt($namePos)));
        }
    }
    protected function checkPropertyHooksForMultiProperty(Property $property, int $hookPos) : void
    {
        if (\count($property->props) > 1) {
            $this->emitError(new \Argtyper202511\PhpParser\Error('Cannot use hooks when declaring multiple properties', $this->getAttributesAt($hookPos)));
        }
    }
    ...
    {
        if (empty($hooks)) {
            $this->emitError(new \Argtyper202511\PhpParser\Error('Property hook list cannot be empty', $this->getAttributesAt($hookPos)));
        }
    }
    protected function checkPropertyHook(PropertyHook $hook, ?int $paramListPos) : void
    {
        ...
        if ($name !== 'get' && $name !== 'set') {
            $this->emitError(new \Argtyper202511\PhpParser\Error('Unknown hook "' . $hook->name . '", expected "get" or "set"', $hook->name->getAttributes()));
        }
        if ($name === 'get' && $paramListPos !== null) {
            $this->emitError(new \Argtyper202511\PhpParser\Error('get hook must not have a parameter list', $this->getAttributesAt($paramListPos)));
        }
    }
    protected function checkPropertyHookModifiers(int $a, int $b, int $modifierPos) : void
    {
        ...
        if ($b != \Argtyper202511\PhpParser\Modifiers::FINAL) {
            $this->emitError(new \Argtyper202511\PhpParser\Error('Cannot use the ' . \Argtyper202511\PhpParser\Modifiers::toString($b) . ' modifier on a property hook', $this->getAttributesAt($modifierPos)));
        }
    }
    protected function checkConstantAttributes(Const_ $node) : void
    {
        if ($node->attrGroups !== [] && \count($node->consts) > 1) {
            $this->emitError(new \Argtyper202511\PhpParser\Error('Cannot use attributes on multiple constants at once', $node->getAttributes()));
        }
    }
    /**
     * @param Property|Param $node
     */
    protected function addPropertyNameToHooks(\Argtyper202511\PhpParser\Node $node) : void
    {
