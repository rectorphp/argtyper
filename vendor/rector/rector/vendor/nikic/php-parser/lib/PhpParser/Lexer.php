...
class Lexer
{
     ...
     * @return Token[] Tokens
     */
    public function tokenize(string $code, ?\Argtyper202511\PhpParser\ErrorHandler $errorHandler = null) : array
    {
        if (null === $errorHandler) {
            $errorHandler = new \Argtyper202511\PhpParser\ErrorHandler\Throwing();
        }
        $scream = \ini_set('xdebug.scream', '0');
        $tokens = @\Argtyper202511\PhpParser\Token::tokenize($code);
    ...
    private function handleInvalidCharacter(\Argtyper202511\PhpParser\Token $token, \Argtyper202511\PhpParser\ErrorHandler $errorHandler) : void
    {
        $chr = $token->text;
        if ($chr === "\x00") {
        ...
        $errorHandler->handleError(new \Argtyper202511\PhpParser\Error($errorMsg, ['startLine' => $token->line, 'endLine' => $token->line, 'startFilePos' => $token->pos, 'endFilePos' => $token->pos]));
    }
    private function isUnterminatedComment(\Argtyper202511\PhpParser\Token $token) : bool
    {
     ...
     */
    protected function postprocessTokens(array &$tokens, \Argtyper202511\PhpParser\ErrorHandler $errorHandler) : void
    {
        ...
        if ($numTokens === 0) {
            ...
            $tokens[] = new \Argtyper202511\PhpParser\Token(0, "\x00", 1, 0);
            return;
        }
        ...
        if ($this->isUnterminatedComment($lastToken)) {
            $errorHandler->handleError(new \Argtyper202511\PhpParser\Error('Unterminated comment', ['startLine' => $lastToken->line, 'endLine' => $lastToken->getEndLine(), 'startFilePos' => $lastToken->pos, 'endFilePos' => $lastToken->getEndPos()]));
        }
        // Add sentinel token.
        $tokens[] = new \Argtyper202511\PhpParser\Token(0, "\x00", $lastToken->getEndLine(), $lastToken->getEndPos());
    }
}
