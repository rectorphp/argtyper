...
final class Modifiers
{
    ...
    public static function verifyClassModifier(int $a, int $b): void
    {
        ...
        if (($a & $b) != 0) {
            throw new \Argtyper202511\PhpParser\Error('Multiple ' . self::toString($b) . ' modifiers are not allowed');
        }
        if ($a & 48 && $b & 48) {
            throw new \Argtyper202511\PhpParser\Error('Cannot use the final modifier on an abstract class');
        }
    }
    /**
     * @internal
    ...
    {
        assert(self::isValidModifier($b));
        if ($a & \Argtyper202511\PhpParser\Modifiers::VISIBILITY_MASK && $b & \Argtyper202511\PhpParser\Modifiers::VISIBILITY_MASK || $a & \Argtyper202511\PhpParser\Modifiers::VISIBILITY_SET_MASK && $b & \Argtyper202511\PhpParser\Modifiers::VISIBILITY_SET_MASK) {
            throw new \Argtyper202511\PhpParser\Error('Multiple access type modifiers are not allowed');
        }
        if (($a & $b) != 0) {
            throw new \Argtyper202511\PhpParser\Error('Multiple ' . self::toString($b) . ' modifiers are not allowed');
        }
        if ($a & 48 && $b & 48) {
            throw new \Argtyper202511\PhpParser\Error('Cannot use the final modifier on an abstract class member');
        }
    }
}
