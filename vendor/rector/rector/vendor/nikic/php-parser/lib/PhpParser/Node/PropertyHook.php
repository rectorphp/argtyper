...
class PropertyHook extends NodeAbstract implements \Argtyper202511\PhpParser\Node\FunctionLike
{
    ...
    public function __construct($name, $body, array $subNodes = [], array $attributes = [])
    {
        ...
        $this->name = \is_string($name) ? new \Argtyper202511\PhpParser\Node\Identifier($name) : $name;
        $this->body = $body;
    ...
    {
        if ($this->body instanceof \Argtyper202511\PhpParser\Node\Expr) {
