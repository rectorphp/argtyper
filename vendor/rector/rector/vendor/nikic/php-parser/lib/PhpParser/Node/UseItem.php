...
class UseItem extends NodeAbstract
{
    ...
    public function __construct(Node\Name $name, $alias = null, int $type = Use_::TYPE_UNKNOWN, array $attributes = [])
    {
        ...
        $this->type = $type;
        $this->name = $name;
        $this->alias = \is_string($alias) ? new \Argtyper202511\PhpParser\Node\Identifier($alias) : $alias;
    }
     ...
     */
    public function getAlias() : \Argtyper202511\PhpParser\Node\Identifier
    {
        if (null !== $this->alias) {
            return $this->alias;
        }
        return new \Argtyper202511\PhpParser\Node\Identifier($this->name->getLast());
    }
...
\class_alias(\Argtyper202511\PhpParser\Node\UseItem::class, \Argtyper202511\PhpParser\Node\Stmt\UseUse::class);
