...
class NameContext
{
     ...
     */
    public function __construct(\Argtyper202511\PhpParser\ErrorHandler $errorHandler)
    {
    ...
    public function addAlias(Name $name, string $aliasName, int $type, array $errorAttrs = []): void
    {
        ...
        if (isset($this->aliases[$type][$aliasLookupName])) {
            ...
            $this->errorHandler->handleError(new \Argtyper202511\PhpParser\Error(sprintf('Cannot use %s%s as %s because the name is already in use', $typeStringMap[$type], $name, $aliasName), $errorAttrs));
            return;
        }
    ...
    public function getResolvedName(Name $name, int $type): ?Name
    {
        ...
        if ($type === Stmt\Use_::TYPE_NORMAL && $name->isSpecialClassName()) {
            if (!$name->isUnqualified()) {
                $this->errorHandler->handleError(new \Argtyper202511\PhpParser\Error(sprintf("'\\%s' is an invalid class name", $name->toString()), $name->getAttributes()));
            }
            return $name;
        }
