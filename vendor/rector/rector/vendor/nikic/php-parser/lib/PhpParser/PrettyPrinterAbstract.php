...
use Argtyper202511\PhpParser\Internal\DiffElem;
use Argtyper202511\PhpParser\Internal\Differ;
use Argtyper202511\PhpParser\Internal\PrintableNewAnonClassNode;
use Argtyper202511\PhpParser\Internal\TokenStream;
...
abstract class PrettyPrinterAbstract implements \Argtyper202511\PhpParser\PrettyPrinter
{
    ...
    {
        $this->phpVersion = $options['phpVersion'] ?? \Argtyper202511\PhpParser\PhpVersion::fromComponents(7, 4);
     ...
     */
    protected function pInfixOp(string $class, \Argtyper202511\PhpParser\Node $leftNode, string $operatorString, \Argtyper202511\PhpParser\Node $rightNode, int $precedence, int $lhsPrecedence): string
    {
     ...
     */
    protected function pPrefixOp(string $class, string $operatorString, \Argtyper202511\PhpParser\Node $node, int $precedence, int $lhsPrecedence): string
    {
     ...
     */
    protected function pPostfixOp(string $class, \Argtyper202511\PhpParser\Node $node, string $operatorString, int $precedence, int $lhsPrecedence): string
    {
        $opPrecedence = $this->precedenceMap[$class][0];
        $prefix = '';
        $suffix = '';
    ...
    protected function pFallback(\Argtyper202511\PhpParser\Node $node, int $precedence, int $lhsPrecedence): string
    {
     ...
     * @return string Pretty printed node
     */
    protected function p(\Argtyper202511\PhpParser\Node $node, int $precedence = self::MAX_PRECEDENCE, int $lhsPrecedence = self::MAX_PRECEDENCE, bool $parentFormatPreserved = \false): string
    {
        ...
        foreach ($node->getSubNodeNames() as $subNodeName) {
            ...
            if (!$subNode instanceof \Argtyper202511\PhpParser\Node && $subNode !== null || !$origSubNode instanceof \Argtyper202511\PhpParser\Node && $origSubNode !== null) {
    ...
    protected function pArray(array $nodes, array $origNodes, int &$pos, int $indentAdjustment, string $parentNodeClass, string $subNodeName, ?int $fixup): ?string
    {
        ...
        foreach ($diff as $i => $diffElem) {
            ...
            if ($diffType === DiffElem::TYPE_KEEP || $diffType === DiffElem::TYPE_REPLACE) {
                ...
                if (!$arrItem instanceof \Argtyper202511\PhpParser\Node || !$origArrItem instanceof \Argtyper202511\PhpParser\Node) {
                ...
                if (!$arrItem instanceof \Argtyper202511\PhpParser\Node) {
                ...
                if (!$origArrItem instanceof \Argtyper202511\PhpParser\Node) {
     ...
     */
    protected function pFixup(int $fixup, \Argtyper202511\PhpParser\Node $subNode, ?string $parentClass, int $subStartPos, int $subEndPos): string
    {
        switch ($fixup) {
            ...
            case self::FIXUP_ENCAPSED:
                if (!$subNode instanceof \Argtyper202511\PhpParser\Node\InterpolatedStringPart && !$this->origTokens->haveBraces($subStartPos, $subEndPos)) {
                    return '{' . $this->p($subNode) . '}';
                }
                break;
            default:
     ...
     */
    protected function callLhsRequiresParens(\Argtyper202511\PhpParser\Node $node): bool
    {
        return !($node instanceof \Argtyper202511\PhpParser\Node\Name || $node instanceof Expr\Variable || $node instanceof Expr\ArrayDimFetch || $node instanceof Expr\FuncCall || $node instanceof Expr\MethodCall || $node instanceof Expr\NullsafeMethodCall || $node instanceof Expr\StaticCall || $node instanceof Expr\Array_);
    }
    /**
     ...
     * @return bool Whether parentheses are required
     */
    protected function dereferenceLhsRequiresParens(\Argtyper202511\PhpParser\Node $node): bool
    {
     ...
     * @return bool Whether parentheses are required
     */
    protected function staticDereferenceLhsRequiresParens(\Argtyper202511\PhpParser\Node $node): bool
    {
        return !($node instanceof Expr\Variable || $node instanceof \Argtyper202511\PhpParser\Node\Name || $node instanceof Expr\ArrayDimFetch || $node instanceof Expr\PropertyFetch || $node instanceof Expr\NullsafePropertyFetch || $node instanceof Expr\StaticPropertyFetch || $node instanceof Expr\FuncCall || $node instanceof Expr\MethodCall || $node instanceof Expr\NullsafeMethodCall || $node instanceof Expr\StaticCall || $node instanceof Expr\Array_ || $node instanceof Scalar\String_ || $node instanceof Expr\ClassConstFetch);
    }
    /**
     ...
     * @return bool Whether parentheses are required
     */
    protected function newOperandRequiresParens(\Argtyper202511\PhpParser\Node $node): bool
    {
        if ($node instanceof \Argtyper202511\PhpParser\Node\Name || $node instanceof Expr\Variable) {
            return \false;
        }
    ...
    {
        return ($modifiers & \Argtyper202511\PhpParser\Modifiers::FINAL ? 'final ' : '') . ($modifiers & \Argtyper202511\PhpParser\Modifiers::ABSTRACT ? 'abstract ' : '') . ($modifiers & \Argtyper202511\PhpParser\Modifiers::PUBLIC ? 'public ' : '') . ($modifiers & \Argtyper202511\PhpParser\Modifiers::PROTECTED ? 'protected ' : '') . ($modifiers & \Argtyper202511\PhpParser\Modifiers::PRIVATE ? 'private ' : '') . ($modifiers & \Argtyper202511\PhpParser\Modifiers::PUBLIC_SET ? 'public(set) ' : '') . ($modifiers & \Argtyper202511\PhpParser\Modifiers::PROTECTED_SET ? 'protected(set) ' : '') . ($modifiers & \Argtyper202511\PhpParser\Modifiers::PRIVATE_SET ? 'private(set) ' : '') . ($modifiers & \Argtyper202511\PhpParser\Modifiers::STATIC ? 'static ' : '') . ($modifiers & \Argtyper202511\PhpParser\Modifiers::READONLY ? 'readonly ' : '');
    }
    ...
    protected function initializeNodeListDiffer(): void
    {
        ...
        $this->nodeListDiffer = new \Argtyper202511\PhpParser\Internal\Differ(function ($a, $b) {
            if ($a instanceof \Argtyper202511\PhpParser\Node && $b instanceof \Argtyper202511\PhpParser\Node) {
