...
class NodeDumper
{
    ...
    protected function dumpRecursive($node, bool $indent = \true): void
    {
        ...
        if ($node instanceof \Argtyper202511\PhpParser\Node) {
            ...
            $this->res .= "{$this->nl})";
        } elseif ($node instanceof \Argtyper202511\PhpParser\Comment) {
    ...
    {
        $strs = [];
        if ($flags & \Argtyper202511\PhpParser\Modifiers::PUBLIC) {
            $strs[] = 'PUBLIC';
        }
        if ($flags & \Argtyper202511\PhpParser\Modifiers::PROTECTED) {
            $strs[] = 'PROTECTED';
        }
        if ($flags & \Argtyper202511\PhpParser\Modifiers::PRIVATE) {
            $strs[] = 'PRIVATE';
        }
        if ($flags & \Argtyper202511\PhpParser\Modifiers::ABSTRACT) {
            $strs[] = 'ABSTRACT';
        }
        if ($flags & \Argtyper202511\PhpParser\Modifiers::STATIC) {
            $strs[] = 'STATIC';
        }
        if ($flags & \Argtyper202511\PhpParser\Modifiers::FINAL) {
            $strs[] = 'FINAL';
        }
        if ($flags & \Argtyper202511\PhpParser\Modifiers::READONLY) {
            $strs[] = 'READONLY';
        }
        if ($flags & \Argtyper202511\PhpParser\Modifiers::PUBLIC_SET) {
            $strs[] = 'PUBLIC_SET';
        }
        if ($flags & \Argtyper202511\PhpParser\Modifiers::PROTECTED_SET) {
            $strs[] = 'PROTECTED_SET';
        }
        if ($flags & \Argtyper202511\PhpParser\Modifiers::PRIVATE_SET) {
            $strs[] = 'PRIVATE_SET';
        }
        if ($strs) {
     ...
     */
    protected function dumpPosition(\Argtyper202511\PhpParser\Node $node): ?string
    {
