...
namespace Argtyper202511\Rector\Php83\Rector\BooleanAnd;
...
CODE_SAMPLE
    ...
    public function matchJsonValidateArg(BooleanAnd $booleanAnd): ?FuncCall
    {
        ...
        $decodeMatch = $this->binaryOpManipulator->matchFirstAndSecondConditionNode($booleanAnd->left, function (Node $node): bool {
            return $node instanceof FuncCall && $this->isName($node->name, 'json_decode');
        }, function (Node $node): bool {
            return $node instanceof ConstFetch && $this->isName($node->name, 'null');
        });
        ...
        $errorMatch = $this->binaryOpManipulator->matchFirstAndSecondConditionNode($booleanAnd->right, function (Node $node): bool {
            return $node instanceof FuncCall && $this->isName($node->name, 'json_last_error');
        }, function (Node $node): bool {
            return $node instanceof ConstFetch && $this->isName($node->name, 'JSON_ERROR_NONE');
        });
