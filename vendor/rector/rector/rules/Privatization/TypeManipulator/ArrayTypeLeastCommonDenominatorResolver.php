...
final class ArrayTypeLeastCommonDenominatorResolver
{
    ...
    public function sharedArrayStructure(Type ...$types): Type
    {
        ...
        $allConstantArrayTypes = array_reduce($types, function ($c, $t): bool {
            return $c && $t instanceof ConstantArrayType;
        }, \true);
        if ($allConstantArrayTypes) {
            /** @var ConstantArrayType[] $consts */
            $consts = $types;
            // Compare key sets (by stringified key types)
            $firstKeys = array_map(function (Type $type): string {
                return $type->describe(VerbosityLevel::typeOnly());
            }, $consts[0]->getKeyTypes());
            foreach ($consts as $c) {
                $keys = array_map(function (Type $type): string {
                    return $type->describe(VerbosityLevel::typeOnly());
                }, $c->getKeyTypes());
                if ($keys !== $firstKeys) {
        ...
        $itemTypes = array_map(function (ArrayType $arrayType): Type {
