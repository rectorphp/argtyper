...
namespace Argtyper202511\Rector\EarlyReturn\Rector\Return_;
...
use Argtyper202511\Rector\EarlyReturn\ValueObject\BareSingleAssignIf;
    ...
    public function refactor(Node $node): ?StmtsAwareInterface
    {
        ...
        foreach ($node->stmts as $key => $stmt) {
            ...
            if (($stmt instanceof For_ || $stmt instanceof Foreach_ || $stmt instanceof While_ || $stmt instanceof Do_) && $initialAssign instanceof Assign) {
                $isReassignInLoop = (bool) $this->betterNodeFinder->findFirst($stmt, function (Node $node) use ($initialAssign): bool {
                    return $node instanceof Assign && $this->nodeComparator->areNodesEqual($node->var, $initialAssign->var);
                });
                if ($isReassignInLoop) {
                    return null;
                }
