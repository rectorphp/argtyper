...
namespace Argtyper202511\Rector\Php81\Rector\MethodCall;
...
CODE_SAMPLE
    ...
    public function refactor(Node $node) : ?Node
    {
        ...
        if (!$this->isObjectType($node->class, new ObjectType('Argtyper202511\\MyCLabs\\Enum\\Enum'))) {
            return null;
        }
    ...
    private function isCallerClassEnum($node) : bool
    {
        if ($node instanceof StaticCall) {
            return $this->isObjectType($node->class, new ObjectType('Argtyper202511\\MyCLabs\\Enum\\Enum'));
        }
        return $this->isObjectType($node->var, new ObjectType('Argtyper202511\\MyCLabs\\Enum\\Enum'));
    }
    ...
    {
        if (!$this->isObjectType($expr, new ObjectType('Argtyper202511\\MyCLabs\\Enum\\Enum'))) {
            return null;
        }
        return $expr;
    }
    private function getEnumConstFetch(StaticCall $staticCall) : ?\Argtyper202511\PhpParser\Node\Expr\ClassConstFetch
    {
        $className = $this->getName($staticCall->class);
        if ($className === null) {
            return null;
    ...
    {
        if (!$this->isObjectType($methodCall->var, new ObjectType('Argtyper202511\\MyCLabs\\Enum\\Enum'))) {
            return null;
        }
