...
final class GitHubOutputFormatter implements OutputFormatterInterface
{
    ...
    private function sanitizeAnnotationProperties(array $annotationProperties) : string
    {
        ...
        $nonNullProperties = \array_filter($annotationProperties, static function ($value) : bool {
            return $value !== null;
        });
        $sanitizedProperties = \array_map(function (string $key, $value) : string {
            return \sprintf('%s=%s', $key, $this->sanitizeAnnotationProperty($value));
        }, \array_keys($nonNullProperties), $nonNullProperties);
