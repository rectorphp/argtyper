...
namespace Argtyper202511\Rector\FileSystem;
...
final class FilesFinder
{
    ...
    public function __construct(\Argtyper202511\Rector\FileSystem\FilesystemTweaker $filesystemTweaker, UnchangedFilesFilter $unchangedFilesFilter, \Argtyper202511\Rector\FileSystem\FileAndDirectoryFilter $fileAndDirectoryFilter, PathSkipper $pathSkipper, \Argtyper202511\Rector\FileSystem\FilePathHelper $filePathHelper, ChangedFilesDetector $changedFilesDetector)
    {
    ...
    public function findInDirectoriesAndFiles(array $source, array $suffixes = [], bool $sortByName = \true, ?string $onlySuffix = null): array
    {
        ...
        if ($hasOnlySuffix) {
            ...
            $fileWithSuffixFilter = static function (string $filePath) use ($onlySuffix): bool {
                return substr_compare($filePath, $onlySuffix, -strlen($onlySuffix)) === 0;
            };
        } elseif ($suffixes !== []) {
            ...
            $fileWithSuffixFilter = function (): bool {
                return \true;
            };
        }
        $filteredFilePaths = array_filter($filteredFilePaths, $fileWithSuffixFilter === null ? function ($value, $key): bool {
            return !empty($value);
        } : $fileWithSuffixFilter, $fileWithSuffixFilter === null ? \ARRAY_FILTER_USE_BOTH : 0);
